    ...
    <section <?php if($section_id): ?> id="<?php echo $section_id;?>" <?php endif; ?> <?php if($section_bg_color): ?>style="background-color: <?php echo $section_bg_color; ?>"<?php endif; ?> class="section-info-boxes <?php if($padding_size): echo $padding_size['value']; endif; ?> <?php if(!$padding_top_desktop): echo ' p-lg-t0'; endif; if(!$padding_bottom_desktop): echo ' p-lg-b0'; endif; if(!$padding_top_mobile): echo ' p-mv-t0'; endif; if(!$padding_bottom_mobile): echo ' p-mv-b0'; endif; ?>">
         <div class="container">        
            <div class="row justify-center"> 
                ...
                <?php if ($subheadline) : ?>
                        <div class="subheadline text-uppercase cl-navy pb-17px text-uppercase wow fadeInUp" data-wow-duration="0.7s" data-wow-delay="0.2s"><?php echo $subheadline; ?></div>
                    <?php endif; ?>      
                    <?php if ($headline) : ?>
                    ...
                    <?php endif; ?>    
                    <?php if ($description) : ?>
                            <div class="dp-1 cl-navy wow fadeInUp" data-wow-duration="0.7s" data-wow-delay="0.2s"><?php echo wp_kses_post($description); ?></div>
                        <?php endif; ?> 
                </div>                         
            </div>  
            <?php if (have_rows('boxes_list')) : ?>
                <div class="row justify-center">
                    <?php while (have_rows('boxes_list')) : the_row(); $box_bg_color = get_sub_field('box_bg_color');   ?>
                        <div class="col-xs-12 <?php echo $columnsnumber;?> col-info-box mt-lg-3 m-t2">
                           <?php if (have_rows('box_info')) : ?>
                           <div class="box-info h-100 w-100" <?php if($box_bg_color): ?> style="background-color: <?php echo $box_bg_color;?>;" <?php endif; ?>>
                            <?php while (have_rows('box_info')) : the_row(); 
                            $title = get_sub_field('title'); 
                            $text = get_sub_field('text'); 
                            $add_padding_top = get_sub_field('add_padding_top'); ?>
                              <div class="content-box <?php if($add_padding_top): ?> p-t1 <?php endif; ?>">
                              <?php if ($title) : ?>
                                <h3 class="cl-navy text-uppercase mt-0 mb-10px wow fadeInUp" data-wow-duration="0.7s" data-wow-delay="0.2s"><?php echo $title; ?></h3>
                            <?php endif; ?>    
                            <?php if ($text) : ?>
                                <div class="dp-1 cl-navy wow fadeInUp" data-wow-duration="0.7s" data-wow-delay="0.2s"><?php echo wp_kses_post($text); ?></div>
                            <?php endif; ?> 
                            <?php if (have_rows('button_list')) {  ?> 
                                <div class="button-list-info start-xs m-t1 wow fadeInUp" data-wow-duration="0.7s" data-wow-delay="0.2s">  
