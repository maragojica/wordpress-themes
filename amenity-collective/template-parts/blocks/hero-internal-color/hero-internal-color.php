<?php
$hero = get_field('hero_internal_color');
if ($hero) {
...
<section class="hero-section max-w-full h-[9em] sm:h-[25em] lg:h-[451px] relative <?php if($add_border_bottom):?> border-b-[12px] border-b-secondary <?php endif;?> <?php if($banner_type['value'] == "image"): if(!empty($image)):?> bg-cover bg-center bg-no-repeat <?php endif; endif; ?> <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?> <?php if(!empty($image)):?>style="background-image: url(<?php echo esc_url($image['url']); ?>)"<?php endif; ?>>
    <?php if($banner_type['value'] == "video"){ 
        $videomp4 = $hero['banner_video_mp4'];  
        $poster_video = $hero['video_poster']; ?> 
        <video class="absolute top-0 left-0 h-full w-full object-cover z-[1]" aria-hidden="true" autoplay loop muted preload playsinline style="pointer-events: none;" <?php if( $poster_video ): ?> poster="<?php echo esc_url($poster_video['url']); ?>" <?php endif; ?>>
            <?php if($videomp4): ?>
    ...
    <div class="overlay <?php echo esc_attr($container_classes); ?> absolute z-[2] top-0 left-0 px-0" <?php if($bg_color):?> style="background-color: <?php echo $bg_color;?>" <?php endif; ?>>
     <div class="container mx-auto">  
            ...
            if($svg_code_icon): ?>
             ...
             <?php endif; endif; ?>   
             <?php if ($subheading) : ?>
                <h3 class="text-white mb-[15px] bodymedium animate__animated" data-animation="fadeIn" data-duration="1.3s">
                    <?php echo $subheading; ?>
                </h3>
            <?php endif; ?>
            <?php if ($heading) : ?>
