...
<section class="hero-internal-section max-w-full relative <?php if($bg_color): echo $bg_class; endif; ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?> <?php echo esc_attr($className); ?> " <?php echo $anchor_attr; ?>> 
    <div class="box-media-hero  w-full lg:h-[369px] md:h-[20em] h-[15em] relative flex items-stretch bg-cover bg-no-repeat bg-center" <?php if($media_type['value'] == "image"){ if($image): ?>style="background-image: url('<?php echo esc_url($image['url']); ?>');" <?php endif; } ?>>
    <?php if($media_type['value'] == "video"){   ?>                                
        ...
        <?php } ?>  
        <div class="box-overlay absolute top-0 left-0 h-full w-full z-[2] pt-[3em] lg:pt-[74px] bg-[rgba(94,110,172,0.7)]">
            <div class="container mx-auto">
                 <div class="w-full lg:max-w-[80%] xl:max-w-[60%] mx-auto text-center">
                    <?php if ($heading) : ?>
    ...
    <?php if($heading || $description || ($buttons && !empty($buttons)) ): ?>              
                <div class="container mx-auto -mt-[40px] lg:-mt-[130px] relative z-[2]" data-aos="fade-in">
                    <div class="bg-white rounded-[10px] lg:py-[75px] lg:px-[121px] md:px-[3em] md:py-[3em] px-[2em] py-[2em] shadow-custom text-center">
                                                    
                            <?php if($description): ?>
                            <div class="text-large text-foreground text-center style-disc" data-aos="fade-up" >                 
                                <?php echo $description; ?>                   
                            </div>
                        <?php endif; ?> 
                         <?php if($subheading): ?>
                            <div class="eyebrow text-quaternary mt-[20px] text-center">
                                <?php echo $subheading; ?>
                            </div>   
                        <?php endif; ?> 
                            <?php if ($buttons) : ?>
                            <div class="flex flex-wrap justify-center gap-2 lg:gap-8 mt-6 justify-center" data-aos="fade-up">
