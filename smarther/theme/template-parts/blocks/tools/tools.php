...
$content_block = get_field('content_block_tools');
if ($content_block) {
...
<section class="tool-section max-w-full relative overflow-hidden <?php if($bg_color): echo $bg_class; endif; ?> <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?>>
    ...
    <div class="container mx-auto relative z-[2]">
        ...
        <!-- Tools Grid -->
        <?php if ($tools) : ?>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 md:gap-x-[50px] gap-y-8 lg:gap-y-[80px] grid-tools" data-aos="fade-up">
               <?php foreach ($tools as $column): 
                   $title = $column['title'];
                   $buttons_cta = $column['buttons_cta'];
                ?>
                
                <div class="event-card flex flex-col h-full justify-between align-items-center">  
                    <div class="event-conten bg-neutral group pt-[35px] lg:pt-[68px] px-[34px] pb-[35px] lg:pb-[53px] flex flex-col flex-grow rounded-[10px]">  
