<?php
$content_block = get_field('content_block_full_banner');
if ($content_block) {
    ...
    $add_overlay = $content_block['add_overlay'];
...
<section class="full-banner-section max-w-full relative bg-foreground <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?> <?php echo esc_attr($className); ?>" <?php echo $anchor_attr; ?> >
    <div class="w-full bg-cover bg-no-repeat bg-center lg:h-[300px] md:h-[30em] sm:h-[25px] h-[20em] overflow-hidden <?php if($add_parallax):?> bg-fixed <?php endif; ?>" <?php if(!empty($image)): ?> style="background-image: url('<?php echo esc_url($image['url']); ?>');" <?php endif; ?>>
      <div class="box-media-hero w-full h-full flex flex-col justify-center items-center text-center relative z-[2]" <?php if($add_overlay): ?> style="background: rgba(26, 27, 35, 0.5); " <?php endif; ?> >
           <?php if($heading || $description || ($buttons && !empty($buttons)) ): ?>              
                <div class="container mx-auto" data-aos="fade-in">
                            <?php if($subheading): ?>
                                <div class="eyebrow text-white mb-[12px]"  >
                                    <?php echo $subheading; ?>
                                </div>   
                            <?php endif; ?>   
                            <?php if ($heading) : ?>
