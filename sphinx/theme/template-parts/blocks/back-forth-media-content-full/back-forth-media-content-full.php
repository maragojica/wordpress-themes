...
if ($content_block) {
    ...
    switch ($bg_color) {
        case 'white':
            ...
            break;
        case 'dark':
            $bg_class = 'bg-foreground';
            break;
        case 'gradient':
            $bg_class = 'bg-gradient-custom';
            break;
        default:
            $bg_class = 'bg-foreground';
            break;
...
<div class="max-w-full w-full z-[2] relative pl-contain pr-contain">
    <div class="w-full h-[1px] bg-[#F1F1F1]"></div>
</div>
<?php endif; ?>
<section class="back-foth-media-content-full-section max-w-full relative overflow-hidden z-[2] <?php if($bg_color): echo $bg_class; endif; ?> <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?>>   
<?php if($main_heading): ?>         
<div class="container mx-auto pb-[3em]">             
    <div class="relative flex items-center justify-center text-center">
        <h2 class="<?php echo esc_attr($text_class); ?> mb-[40px]" data-aos="fade-in">                 
...
<?php endif; ?>
     <?php $i = 1; if( $list_content ): ?> 
         <?php foreach ($list_content as $column): 
            ...
            $heading_type = $column['heading_type'];
            $subheading = $column['subheading']; 
            $add_list_icons = $column['add_list_icons']; 
            $list_icons = $column['list_icons']; 
            $description = $column['description']; 
            $buttons = $column['buttons'];   
             $add_media_shape = $column['add_media_shape']; 
            ...
            $content_width = '';
                ...
                switch ($columns_gap) {
                    case 'small':
                        if ($reverse_desktop) {
                            $columns_gap_class = ' lg:pl-[50px] xl:pl-[77px] lg:pr-0';
                        } else {
                            $columns_gap_class = ' lg:pr-[50px] xl:pl-[77px] lg:pl-0';
                        }
                        break;
                    case 'medium':
                        if ($reverse_desktop) {
                            $columns_gap_class = ' lg:pl-[60px] xl:pl-[100px] lg:pr-0';
                        } else {
                            $columns_gap_class = ' lg:pr-[60px] xl:pr-[100px] lg:pl-0';
                        }
                        break;
                    case 'large':
                        if ($reverse_desktop) {
                            $columns_gap_class = ' lg:pl-[70px] xl:pl-[170px] lg:pr-0';
                        } else {
                            $columns_gap_class = ' lg:pr-[70px] xl:pr-[170px] lg:pl-0';
                        }
                        break;
                    default:   
                     if ($reverse_desktop) {
                ...
                switch ($row_gap) {
                    case 'small':
                        $row_gap_class = ' lg:py-[23px] ';
                        break;
                    case 'medium':
                        $row_gap_class = ' lg:py-[45px] ';
                        break;
                    case 'large':
        ...
        <div class="content-box relative z-2 w-full"> 
          <div class="w-full mx-auto <?php if($reverse_desktop){ echo " pr-contain not-rp";}else{ echo " pl-contain not-lp";} ?>">   
           <div class="<?php echo esc_attr($container_classes); ?> lg:gap-y-0 gap-y-[3em] <?php if($i > 1){ echo ' mt-[3em] lg:mt-[90px]'; } ?>">               
              <div class="<?php echo esc_attr($heading_width); ?> px-[2rem] <?php echo $row_gap_class; ?> <?php echo $columns_gap_class; ?>  justify-center">
                    <?php if($subheading): ?>
                        <div class="eyebrow <?php echo esc_attr($text_class); ?> mb-[12px]"  >
                            <?php echo $subheading; ?>
                        </div>   
                    <?php endif; ?> 
                        <?php if ($heading) : ?>
                        <?php if($heading_type === 'headthree'): ?>
                            <h3 class="<?php echo esc_attr($text_class); ?> mb-0" >
                                <?php echo $heading; ?>
                            </h3>
                        <?php elseif($heading_type === 'headtwo'): ?>
                            <h2 class="<?php echo esc_attr($text_class); ?> mb-0" >
                                <?php echo $heading; ?>
                            </h2> 
                    <?php endif; endif; ?> 
                    <?php if ($add_list_icons): ?>                        
                    <div class="pt-[20px]">
                        <?php if($list_icons): ?>
                      <?php foreach ($list_icons as $column):       
                        $text = $column['text'];      
                        $icon = $column['icon'];  ?>
                        <div class="flex flex-row gap-x-[25px] items-center pt-[10px]">
                             <?php if($icon): ?>
                                <img src="<?php echo esc_url($icon['url']); ?>" alt="<?php echo esc_attr($icon['alt'] ? $icon['alt'] : $text); ?>" class="transition-all duration-300 h-[60px] lg:h-[100px] object-contain" />
                            <?php endif; ?>
                             <?php if($text): ?>
                                <h4 class="text-white mb-0"><?php echo $text;?></h4>
                            <?php endif; ?> 
                        </div>
                     <?php endforeach; ?>
                      <?php endif; ?>
                    ...
                    <?php endif; ?>
                       <?php if($description): ?>
                        <div class="<?php echo esc_attr($text_size_class); ?> <?php echo esc_attr($text_class); ?> style-disc mt-5 lg:mt-[30px]" data-aos="fade-in" >                 
                            <?php echo $description; ?>                   
                        </div>
                    <?php endif; ?>                
                    <?php if ($buttons) : ?>
                        <div class="flex flex-wrap gap-2 lg:gap-8 mt-5 lg:mt-[30px] justify-start" data-aos="fade-up">
                ...
                <div class="<?php echo esc_attr($content_width); ?>" <?php if($reverse_desktop){ ?> data-aos="fade-left" <?php }else{ ?> data-aos="fade-right" <?php } ?>  >  
                    <div class="media-box w-full lg:h-full md:h-[25em] sm:h-[20em] h-[15em] relative bg-cover bg-top bg-no-repeat <?php if($reverse_desktop){ echo 'rounded-right'; }else{ echo 'rounded-left'; } ?> <?php if($add_media_shape): if($reverse_desktop){ echo ' shape-media-top-r'; }else{ echo ' shape-media-bottom-l';} endif; ?>" <?php if($media_type['value'] == "image"){ ?> style="background-image: url('<?php echo esc_url($image['url']); ?>');" <?php } ?>>
                       <?php if($add_media_shape): ?>
                        <?php if($reverse_desktop){ ?>                              
                            <svg class="absolute top-[-1px] right-[-2px] w-[50px] h-[82px] z-[4]" xmlns="http://www.w3.org/2000/svg" width="50" height="84" viewBox="0 0 50 84" fill="none">
                            <path d="M47.5375 76.5354C48.394 78.5911 50 83.0308 50 83.6018L50 3.73243C50 2.07558 48.6569 0.732429 47 0.732429L7.24467e-06 0.732425C2.05568 0.732425 4.92506 3.87304 6.10279 5.44335C19.7359 28.8195 46.6809 74.4798 47.5375 76.5354Z" fill="#1A1B23"/>
                            </svg>                       
                        <?php }else{ ?>                           
                            <svg class="absolute bottom-[-1px] left-[-2px] w-[50px] h-[82px] z-[4]" xmlns="http://www.w3.org/2000/svg" width="51" height="83" viewBox="0 0 51 83" fill="none">
                            <path d="M3.4422 7.09446C2.58567 5.03878 0.979675 0.599097 0.979675 0.0280762V79.8975C0.979675 81.5543 2.32282 82.8975 3.97968 82.8975H50.9797C48.924 82.8975 46.0546 79.7568 44.8769 78.1865C31.2438 54.8104 4.29873 9.15013 3.4422 7.09446Z" fill="#1A1B23"/>
                            </svg>
                        <?php } ?>  
                        <?php endif; ?>
   ...
   <?php $i++; endforeach; endif; ?>
       <?php if ($buttons_general) : ?>
        <div class="container mx-auto pt-[4em]"> 
                        <div class="flex flex-wrap gap-2 lg:gap-8 mt-5 lg:mt-[30px] justify-center" data-aos="fade-up">
                            <?php foreach ($buttons_general as $button) : ?>
                                <?php 
                                $button_link = $button['button'];
                                $button_style = $button['button_style'];
                                $is_external = $button['is_external'];
