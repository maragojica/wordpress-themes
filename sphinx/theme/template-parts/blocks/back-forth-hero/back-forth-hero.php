<?php
$content_block = get_field('content_block_media_content_hero');
if ($content_block) {
...
<section class="back-forth-media-hero-section max-w-full w-full relative overflow-hidden z-[2]  bg-foreground bg-cover bg-no-repeat bg-center <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?> <?php if(!empty($bg_image)){ ?> style="background-image: url('<?php echo esc_url($bg_image['url']); ?>');" <?php } ?>>
  <div class="w-full h-full relative px-[2rem] lg:px-0">  
    <?php if($main_heading): ?>
    <div class="box-heading w-full z-[4] pt-[4em] lg:pt-[100px] lg:absolute top-0 left-0">
        <div class="pl-contain not-lp mx-auto relative">             
        <div class="relative w-full lg:w-[45%] text-left">
            <h1 class="text-white" data-aos="fade-in">                 
    ...
    <?php endif; ?>
    <?php if($heading || $description || $buttons): ?>
        <div class="box-content w-full lg:max-w-[80%] xl:max-w-[70%] bg-gradient-fade-l ml-auto pb-[3em] lg:py-[100px] pr-contain not-rp flex">
            <div class="lg:w-[30%] xl:w-[40%]"></div>
            <div class="lg:w-[70%] xl:w-[60%]">
                 <?php if($main_heading): ?>
                <div class="invisible headline text-white mb-[80px] lg:block hidden" data-aos="fade-in"><?php echo $main_heading; ?></div>
                <?php endif; ?>
                <?php if($subheading): ?>
                    <div class="eyebrow text-white mb-[12px]" data-aos="fade-up">
                        <?php echo $subheading; ?>
                    </div>  
                <?php endif; ?> 
                    <?php if ($heading) : ?>
                ...
                <?php endif; ?>                     
                <?php if($description): ?>
                <div class="text-default text-white style-disc mt-5 lg:mt-[30px]" data-aos="fade-up" data-aos-delay="100" >                 
  ...
  <?php if(!empty($bg_image)){ ?>
        <div class="w-full md:h-[30em] sm:h-[25em] h-[20em] relative bg-cover bg-center bg-no-repeat lg:hidden block" style="background-image: url('<?php echo esc_url($bg_image['url']); ?>')"></div>
    <?php } ?>
</section>
