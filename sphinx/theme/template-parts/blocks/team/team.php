...
if ($content_block) {
    ...
    //Info Container Settings
     ...
     $info_ontainer_classes = 'w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-y-[30px] md:gap-x-[25px] md:gap-y-[50px] justify-items-center ';
?>
      ...
      <div class="container mx-auto"> 
          <div class="<?php echo esc_attr($container_classes); ?>">
              <div class="w-full mx-auto text-center">
                  <?php if($subheading): ?>
                        <div class="eyebrow <?php echo esc_attr($text_class); ?>  mb-0" data-aos="fade-up">
                            <?php echo $subheading; ?>
                        </div>  
                    <?php endif; ?> 
                        <?php if ($heading) : ?>
                        <h2 class="<?php echo esc_attr($text_class); ?> mb-0" data-aos="fade-up" >
                            <?php echo $heading; ?>
                        </h2> 
                    <?php endif; ?> 
                        <?php if($description): ?>
                        <div class="text-large <?php echo esc_attr($text_class); ?> style-disc mt-5 lg:mt-[30px] mx-auto lg:max-w-[76%] 2xl:max-w-[65%]" data-aos="fade-up" data-aos-delay="100" >                 
          ...
          <?php if( $team_list ): ?> 
            <div class="max-w-full lg:max-w-[80%] mx-auto">
                <div class="<?php echo esc_attr($info_ontainer_classes); ?> <?php echo ($subheading || $heading || $description) ? ' mt-[3em] lg:mt-[44px]' : ''; ?>">
                 <?php foreach ($team_list as $post): 
                    ...
                    $add_single_page = get_field('add_single_page', $post->ID); ?>
                    <div class="w-full" data-aos="fade-up" >  
                       <?php if( $add_single_page): ?>
                        <a class="w-full h-full block" href="<?php the_permalink($post->ID); ?>" tabindex="0" aria-label="<?php echo $title; ?>" title="<?php echo $title; ?>">
                         <?php endif; ?>   
                         <div class="team-info flex flex-col justify-start w-full h-full text-left group hover:-translate-y-1 transition-transform duration-300">                      
                           <?php 
                                if ( has_post_thumbnail( $post->ID ) ): ?>
                                <div class="w-full h-fit md:h-[34em] lg:h-[330px] 2xl:h-[412px] team-box rounded-[3px] relative mb-[15px] overflow-hidden">
                                ...
                                ?>
                            <?php if($title): ?>
                                <h5 class="<?php echo esc_attr($text_class); ?> mb-0 transition-all duration-400 ease-in-out"><?php echo $title;?></h5>
                            <?php endif; ?> 
                             <?php if($position): ?>
                                <div class="<?php echo esc_attr($text_class); ?> <?php if($title): ?> mt-0 <?php endif;?>" data-aos="fade-up" >                 
                                    <?php echo $position; ?>                   
                                </div>
                            <?php endif; ?>                            
                       </div>
