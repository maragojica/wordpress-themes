<?php
$content_block = get_field('content_block_newspress');
if ($content_block) {
    ...
    $newslist = $content_block['newslist'];     
    $bg_image_overlay = $content_block['bg_image_overlay'];   
...
<section class="news-press-section max-w-full relative bg-foreground overflow-hidden relative z-[1] <?php echo esc_attr($className); ?> " <?php echo $anchor_attr; ?>>
    <div class="w-full h-full <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>">
      <div class="box-media-hero  w-full h-full flex flex-col justify-center items-center text-center relative">         
            <?php if($heading ): ?>
            <div class="top-background bg-cover bg-no-repeat bg-top w-full pt-[3em] lg:pt-[100px] pb-[50px]" <?php if(!empty($bg_image_overlay)): ?> style="background-image: url('<?php echo esc_url($bg_image_overlay['url']); ?>');" <?php endif; ?>>
            ...
            <?php endif; ?> 
           <?php if($subheading || $description || ($buttons && !empty($buttons)) ): ?>       
            <div class="bottom-gradient bg-gradient-custom-reverse w-full pt-[25px] mt-[-50px]">       
                <div class="container mx-auto" data-aos="fade-in">  
                    <?php if($subheading): ?>
                    <h2 class="text-white mb-0" data-aos="fade-up" data-aos-delay="100">
                        <?php echo $subheading; ?>
                    </h2>  
                <?php endif; ?> 
                    <?php if($description): ?>
                    <div class="text-large text-white style-disc mt-5 mx-auto lg:max-w-[70%]" data-aos="fade-up" data-aos-delay="100" >                 
                ...
                <?php endif; ?> 
                    ...
                    <?php endif; ?>  
                    <?php $news_posts = array();

                if ($settings === 'all') {
                    // Query all news posts
                    $args = array(
                        'post_type' => 'news',
                        'posts_per_page' => -1, // Get all news
                        'orderby' => 'date',
                        'order' => 'DESC',
                    );
                ...
                } elseif ($settings === 'featured' && $newslist) {
                    // Use selected posts from newslist (post object field)
                    $news_posts = $newslist;
                }
                $count_allpost = count($news_posts);
                ...
                if ($news_posts): ?>
                    <div class="w-full lg:mt-[76px] mt-[3em] lg:max-w-[90%] mx-auto">
                        ...
                        ?>
                            <div class="w-full flex flex-col lg:flex-row items-stretch mb-[60px] group bg-white <?php if( $count_allpost > 3): ?> news-item <?php endif; ?>" data-aos="fade-in">                           
                                <?php if ($featured_image): ?>
                                    <div class="w-full lg:w-[55%]">
                                        <a href="<?php echo esc_url($permalink); ?>" aria-label="<?php echo esc_html($title); ?>" title="<?php echo esc_html($title); ?>" tabindex="0" class="block w-full lg:h-full md:h-[30em] h-[20em] bg-cover bg-no-repeat bg-center transition-opacity hover:opacity-90" style="background-image: url('<?php echo esc_url($featured_image); ?>');"></a>
                                    </div>
                                <?php endif; ?>
                            
                                <div class="w-full h-full lg:w-[45%] p-[2.5rem] lg:pl-[77px] lg:py-[52px] lg:pr-[46px] flex flex-col justify-center items-start text-left" data-aos="fade-up" <?php if(isset($bg_content) && $bg_content): ?> style="background-color: <?php echo esc_attr($bg_content); ?>;" <?php endif; ?>>    
                                    <h3 class="text-foreground mb-[18px]" data-aos="fade-up">
                                        <a href="<?php echo esc_url($permalink); ?>" class="text-foreground group-hover:text-primary" aria-label="<?php echo esc_html($title); ?>" title="<?php echo esc_html($title); ?>" tabindex="0">
                                            <?php echo esc_html($title); ?>
                                        </a>
                                    </h3>
