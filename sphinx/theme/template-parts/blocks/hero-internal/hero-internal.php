...
<section class="hero-internal-section max-w-full relative bg-foreground overflow-hidden relative z-[1] <?php echo esc_attr($className); ?> " <?php echo $anchor_attr; ?>>
    <div class="w-full h-full <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>">
      <div class="box-media-hero  w-full h-full flex flex-col justify-center items-center text-center relative">         
            <?php if($heading ): ?>
            <div class="top-background bg-cover bg-no-repeat bg-top w-full pt-[4em] lg:pt-[100px] pb-[50px]" <?php if(!empty($bg_image_overlay)): ?> style="background-image: url('<?php echo esc_url($bg_image_overlay['url']); ?>');" <?php endif; ?>>
           ...
           <?php if($subheading || $description || ($buttons && !empty($buttons)) ): ?>       
            <div class="bottom-gradient bg-gradient-custom-reverse w-full pt-[25px] mt-[-50px]">       
                ...
                <?php endif; ?> 
                    <?php if($description): ?>
                    <div class="text-large text-white style-disc mt-5 mx-auto lg:max-w-[60%]" data-aos="fade-up" data-aos-delay="100" >                 
