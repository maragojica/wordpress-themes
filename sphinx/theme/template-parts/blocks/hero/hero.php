...
<section class="hero-section max-w-full relative bg-foreground lg:h-screen md:h-[50em] sm:h-[45em] h-[40em] overflow-hidden <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?> <?php echo esc_attr($className); ?> " <?php echo $anchor_attr; ?>> 
     <?php if($add_shape): ?>
        <div class="absolute top-0 left-0 w-full h-full overflow-hidden z-[1]">
            <div class="svg-container" id="svgContainer">
                <?php 
                $svg = file_get_contents(get_template_directory() . '/shapes/sphinxbg-texture.svg');
                echo $svg;
                ?>
            </div>
        </div>
    <?php endif; ?>
      <div class="box-media-hero  w-full h-full flex flex-col justify-center items-center text-center relative z-[2]">
           <?php if($heading || $description || ($buttons && !empty($buttons)) ): ?>              
                <div class="container mx-auto" data-aos="fade-in">  
                        ...
                        <?php endif; ?> 
                         <?php if($subheading): ?>
                            <h2 class="text-white mt-[40px] mb-0" data-aos="fade-up" data-aos-delay="100">
                                <?php echo $subheading; ?>
                            </h2>  
                        <?php endif; ?> 
                            <?php if($description): ?>
    ...
    <?php if($scroll): 
         $url = $scroll['url'];
         $title = $scroll['title']; ?>
        <div class="absolute bottom-[30px] left-1/2 transform -translate-x-1/2 z-[2]" data-aos="fade-up" data-aos-delay="200">
            <div class="scroll-to-next">
            <a href="<?php echo esc_attr($url); ?>" class="scroll-arrow-btn" aria-label="Scroll to next section">
                <svg width="39" height="39" viewBox="0 0 39 39" fill="none" xmlns="http://www.w3.org/2000/svg">
                <circle cx="19.5" cy="19.3596" r="18.5" stroke="#E39C33"/>
                <g clip-path="url(#clip0_4244_1285)">
                    <mask id="mask0_4244_1285" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="13" y="11" width="13" height="16">
                    <path d="M26 26.8596L26 11.8596L13 11.8596L13 26.8596L26 26.8596Z" fill="white"/>
                    </mask>
                    <g mask="url(#mask0_4244_1285)">
                    <path class="arrow-path" d="M20.3196 24.3433C21.6704 23.0886 22.8528 21.992 24.0352 20.893C24.1999 20.7399 24.3646 20.5871 24.5298 20.4351C24.9063 20.0917 25.3424 19.9363 25.7518 20.3279C26.1405 20.6994 26.0401 21.1299 25.6686 21.4805C24.9249 22.1828 24.1641 22.8696 23.4097 23.5625C22.3795 24.5085 21.3518 25.4574 20.3164 26.3988C19.6542 27.0008 19.3077 27.0153 18.6763 26.4336C16.9289 24.828 15.1946 23.2105 13.4504 21.6017C13.0407 21.2243 12.789 20.7857 13.2365 20.3472C13.7535 19.8403 14.2094 20.1907 14.62 20.5729C15.9123 21.7757 17.2094 22.9741 18.635 24.2954C18.6556 23.8919 18.6877 12.8412 18.7083 12.549C18.7578 11.8596 19.2402 11.8596 19.5538 11.8596C19.6702 11.8596 20.2835 11.8799 20.3254 12.6442C20.3449 13.0039 20.3196 21.009 20.3196 22.7042L20.3196 24.3433Z" fill="#E39C33"/>
                    </g>
                </g>
                <defs>
                    <clipPath id="clip0_4244_1285">
                    <rect width="15" height="13" fill="white" transform="translate(26 11.8596) rotate(90)"/>
                    </clipPath>
                </defs>
                </svg>
            </a>
            <span class="eyebrow"><?php echo $title; ?></span>
            </div>
        </div>
    <?php endif; ?>
</section>
