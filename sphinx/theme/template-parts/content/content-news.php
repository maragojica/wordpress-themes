			 ...
			 <div class="bottom-gradient bg-gradient-custom-reverse w-full pb-[30px] lg:mt-[-100px] mt-[-4em] relative z-[2] relative overflow-hidden">
				<?php if($add_shape): ?>
			      ...
			      <div class="container mx-auto relative z-[2]" data-aos="fade-in"> 
					...
					// Get the taxonomy terms from current post
					$current_terms = get_the_terms($current_post_id, 'news-type');
					$term_ids = array();
					...
					$args = array(
						...
						'posts_per_page' => 2, // Number of articles to show
						'post__not_in' => array($current_post_id), // Exclude current post
						'orderby' => 'date',
						'order' => 'DESC',
					);

					// If current post has terms, get related posts from same terms
					if (!empty($term_ids)) {
