...
$content_block = get_field('content_team_list');
if ($content_block) {
...
<section class="team-list-section max-w-full relative <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?> >
    <div class="container mx-auto">  
        ...
        <?php if ($subheading) : ?>
         ...
         <?php if($team_members): ?>
            <div class="flex relative flex-row flex-wrap mx-auto sm:mx-[-15px]">
            <?php $animation_delay = 1; foreach( $team_members as $post ): 
                  setup_postdata($post); 
                 $title = get_the_title($post->ID); 
                 $position = get_field('position', $post->ID); 
                 $fun_image = get_field('fun_image', $post->ID);
                 $duration = $animation_delay . 's'; ?>
                  <div class="w-full sm:w-1/2 md:w-1/3 lg:w-1/4 mt-[32px] sm:px-[15px] animate__animated" data-animation="fadeBottom" data-duration="<?php echo esc_attr($duration); ?>">
                    <a class="w-full h-full block" href="<?php the_permalink($post->ID); ?>" tabindex="0" aria-label="<?php echo $title; ?>" title="<?php echo $title; ?>">
                        <div class="relative w-full h-[25em] sm:h-[20em] md:h-[310px] lg:h-[310px] xl:h-[360px] group">
                        <!-- Title on top -->
                        <div class="absolute bottom-0 left-0 z-[1] w-full h-full max-h-[123px] bg-opacity-50 py-[15px] px-[22px] flex flex-col justify-end bg-[linear-gradient(180deg,_rgba(0,0,0,0)_13.55%,_#000_93.7%)]">
                        <?php if($title): ?>
             ...
             <?php $animation_delay += 0.01; endforeach; ?>  
           </div>    
