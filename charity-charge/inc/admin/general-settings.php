...
function custom_sync_favicon_with_site_icon()
{
...
add_action('acf/save_post', 'custom_sync_favicon_with_site_icon', 20);
...
function custom_is_turnstile_enabled()
{
    $login_options = get_field('login_customizer', 'option');
...
function custom_wpp_login_script()
{
    if (custom_is_turnstile_enabled()) {
...
add_action('login_enqueue_scripts', 'custom_wpp_login_script');

// Add CF Turnstile on login page
function custom_add_turnstile_on_login_page()
{
    if (custom_is_turnstile_enabled()) {
        $login_options = get_field('login_customizer', 'option');
...
add_action('login_form', 'custom_add_turnstile_on_login_page');

// Validating CF Turnstile on login page
function custom_turnstile_login_check($user, $password)
{
    if (custom_is_turnstile_enabled()) {
...
add_action('wp_authenticate_user', 'custom_turnstile_login_check', 10, 2);
...
function custom_disable_comments_admin()
{
...
add_action('admin_init', 'custom_disable_comments_admin');

function custom_close_comments()
{
    $cleanup_options = get_field('cleanup', 'option');
...
add_filter('comments_open', 'custom_close_comments', 20, 2);
add_filter('pings_open', 'custom_close_comments', 20, 2);

function custom_hide_existing_comments($comments)
{
    $cleanup_options = get_field('cleanup', 'option');
...
add_filter('comments_array', 'custom_hide_existing_comments', 10, 2);

function custom_remove_comments_menu()
{
    $cleanup_options = get_field('cleanup', 'option');
...
add_action('admin_menu', 'custom_remove_comments_menu');

function custom_remove_comments_admin_bar()
{
...
add_action('init', 'custom_remove_comments_admin_bar', PHP_INT_MAX);

function custom_disable_wp_emoji()
{
...
if (function_exists('get_field')) {
    ...
    if (!empty($cleanup_options['disable_emojis'])) {
        add_action('init', 'custom_disable_wp_emoji', 10);
    }
}


function custom_allowed_block_types($block_types, $post)
{
 ...
 */
function custom_block_category($categories, $block_editor_context)
{
 ...
 */
function custom_cleanup_actions()
{
    if (function_exists('get_field')) {
        $cleanup_options = get_field('cleanup', 'option');
...
add_action('init', 'custom_cleanup_actions', PHP_INT_MAX);
