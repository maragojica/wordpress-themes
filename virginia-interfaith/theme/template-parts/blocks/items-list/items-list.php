<?php
$content_block = get_field('content_block_items_list');
if ($content_block) {    
    ...
    $items_list = $content_block['items_list']; 
...
<section class="items-list-section max-w-full relative overflow-hidden <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?>>   
    <div class="container mx-auto relative z-2">        
          <div class="<?php echo esc_attr($container_classes); ?> relative">  
              <div class="w-full lg:max-w-[70%] lg:px-[2rem] text-center relative">                                 
                    <?php if($subheading): ?>
                ...
                <?php if($items_list): ?>
                    ...
                    ?>
                    <div class="bg-accent rounded-[20px] lg:px-[60px] py-[25px] px-[25px] lg:py-[45px] mt-[40px] lg:max-w-[70%] mx-auto" data-aos="fade-in"  >
                        ...
                        <?php endif; ?>  
                        <?php if($list_content): 
                            foreach ($list_content as $items_list) { 
                                $title = $items_list['title'];
                                $text = $items_list['text'];
                                $status = $items_list['status'];
                            ?>
                            <div class="box-action flex gap-x-[20px] flex-row items-stretch mt-[40px]">
                                <!-- Remove the wrapper div, just use the line directly -->
                                <div class="w-[4px] bg-action rounded-[4px]"></div>
                                
                                <div class="flex-1">
                                    <?php if($title): ?>
                                        <h4 class="text-foreground mb-[8px]">
                                            <?php echo $title; ?>
                                        </h4> 
                                    <?php endif; ?>  
                                    <?php if($text): ?>
                                        <div class="text-small text-foreground style-disc">                 
                                            <?php echo $text; ?>                   
                                        </div>
                                    <?php endif; ?>
                                    <?php if($status): ?>
                                        <div class="status flex gap-x-[8px] items-center mt-[11px]">
                                            <svg xmlns="http://www.w3.org/2000/svg" width="18" height="17" viewBox="0 0 18 17" fill="none">
                                                <path d="M9.86885 1.36975C9.70323 1.026 9.35323 0.807251 8.96885 0.807251C8.58448 0.807251 8.2376 1.026 8.06885 1.36975L6.05948 5.50413L1.57198 6.16663C1.19698 6.22288 0.884479 6.48538 0.768854 6.84475C0.653229 7.20413 0.746979 7.601 1.01573 7.86663L4.27198 11.0885L3.50323 15.6416C3.44073 16.0166 3.59698 16.3979 3.90635 16.6198C4.21573 16.8416 4.6251 16.8698 4.9626 16.6916L8.97198 14.551L12.9814 16.6916C13.3189 16.8698 13.7282 16.8448 14.0376 16.6198C14.347 16.3948 14.5032 16.0166 14.4407 15.6416L13.6689 11.0885L16.9251 7.86663C17.1939 7.601 17.2907 7.20413 17.172 6.84475C17.0532 6.48538 16.7439 6.22288 16.3689 6.16663L11.8782 5.50413L9.86885 1.36975Z" fill="#D12338"/>
                                            </svg>
                                            <span class="pt-[3px]"><?php echo $status; ?></span>
                                        </div>
                                    <?php endif; ?>
                                </div>
                            </div> 
