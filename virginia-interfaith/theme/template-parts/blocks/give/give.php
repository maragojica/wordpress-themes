<?php
$content_block = get_field('content_block_give');
if ($content_block) {        
...
<section class="give-section max-w-full relative overflow-hidden <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?>>   
    <div class="container mx-auto relative z-2">  
        <div class="bg-accent rounded-[20px] md:px-[60px] lg:px-[120px] py-[25px] px-[25px] lg:py-[45px]" data-aos="fade-in" >
            <?php if($heading): ?>
                <h3 class="text-foreground text-center mb-[20px]" >
                    <?php echo $heading; ?>
                </h3> 
            <?php endif; ?> 
            <?php if($items_list): ?>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-[16px]">
                    <?php foreach($items_list as $item): 
                        $title_content = $item['title_list'];
                        $icon_content = $item['icon_list'];  ?> 
                        <div class="flex items-center gap-x-[16px]">
                            <?php if($icon_content): ?>
                                <div class="w-[20px] h-[20px] flex items-center justify-center">
                                    <img src="<?php echo esc_url($icon_content['url']); ?>" alt="<?php echo esc_attr($icon_content['alt'] ? $icon_content['alt'] : 'Icon'); ?>" class="w-[20px] h-[20px] object-contain"/>
                                </div>
                            <?php endif; ?>
                            <?php if($title_content): ?>
                                <div class="text-foreground text-small">
