<?php
$content_block = get_field('content_block_get_involved');
if ($content_block) {   
    ...
    $container_classes = ' flex flex-col lg:flex-row items-center lg:items-end  ';   
    ...
    if(!$spacing_bottom_mobile): $spacing_responsive_class .= ' pb-0-important'; endif;
      ...
      $bottom_wave =  '/shapes/bottom-wave-yellow--white-swish.svg';
?>
 <?php if (!$settings) { ?> 
<section class="get-involved-section bg-white max-w-full w-full relative overflow-hidden">
<div class=" max-w-full w-full relative overflow-hidden <?php echo esc_attr($className); ?>" <?php echo $anchor_attr; ?>>   
  
  <!-- Background with gradient overlay -->
  <div class="w-full h-full" <?php if(!empty($image)):?> style="background-image: linear-gradient(rgba(36, 65, 143, 0.9), rgba(36, 65, 143, 0.9)), url('<?php echo esc_url($image['url']); ?>'); background-size: cover; background-position: center;" <?php else: ?> style="background-color: rgba(36, 65, 143, 0.9);" <?php endif; ?>>
  
  <div class="w-full h-full <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>">
    <div class="container mx-auto relative z-10 pb-[168px]">        
          <div class="w-full mx-auto <?php echo esc_attr($container_classes); ?> relative lg:gap-y-0 gap-y-[1em]">  
                <div class="w-full lg:w-3/4 custom-p-info lg:text-left text-center lg:pr-[2em]">             
                <?php if($subheading): ?>
                ...
                <?php endif; ?> 
                    <?php if ($heading) : ?>
                    <h2 class="text-white heading-shape mb-0" >
                        <?php echo $heading; ?>
                    </h2> 
                <?php endif; ?>  
                <?php if($description): ?>
                        <div class="text-white text-small style-disc mt-[15px]" data-aos="fade-up" data-aos-delay="100" >                 
                ...
                <div class="w-full lg:w-1/4">  
                    <?php if ($buttons) : ?>
                        <div class="flex flex-wrap gap-2 lg:gap-[16px] justify-center lg:justify-end " data-aos="fade-up">
 ...
 <div class="bottom-cta-shape bg-transparent relative z-[2] w-full max-w-full leading-[0] mt-[-190px] overflow-hidden relative" data-aos="fade-in">
             <?php 
                $svg = file_get_contents(get_template_directory() . $bottom_wave);
                echo $svg;
                ?>
                <div class="w-full h-[20px] bg-white absolute bottom-[-1px] z-[1]"></div>
        </div>
</section>
 <?php } else { 
            include get_template_directory() . '/template-parts/global/global-get-involved.php'; 
        } ?> 
<?php }
?>
