...
$content_block = get_field('content_block_tabs_resources');
if ($content_block) {    
    ...
    $buttons = $content_block['buttons'];
     $tabs_resources = $content_block['tabs_resources'];
...
<section class="tabs-resources-section max-w-full relative overflow-hidden <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?>>
  

    <div class="container mx-auto relative z-[2]">
        
        <!-- Header Section -->
        <div class="text-left mb-6 mx-auto" data-aos="fade-in">
             <?php if($subheading): ?>
                    ...
                    <?php endif; ?>                     
                    <?php if($description): ?>
                    <div class="text-foreground style-disc style-disc mt-[20px] xl:mt-[30px]" data-aos="fade-in" >                 
                        <?php echo $description; ?>                   
                    </div>
                <?php endif; ?>
        </div>

       <!-- Tabs Navigation (Desktop) & Dropdown (Mobile) -->
<?php if ($tabs_resources) : ?>
    <!-- Mobile Dropdown -->
    <div class="lg:hidden mb-8 relative z-[1]" data-aos="fade-up">
        <div class="relative">
            <button type="button" class="tabs-dropdown-trigger w-full bg-accent text-foreground px-6 py-4 rounded-[20px] flex justify-between items-center font-bold">
                <span class="selected-tab-title">Select a category</span>
                <svg class="dropdown-arrow w-5 h-5 transition-transform" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7" />
                </svg>
            </button>
            <div class="tabs-dropdown-menu absolute w-full mt-2 bg-accent rounded-[20px] shadow-lg hidden z-10">
                <?php foreach ($tabs_resources as $index => $tab): 
                    $tab_title = $tab['tab_title'];
                ?>
                    <button type="button" class="tab-option w-full text-left px-6 py-4 hover:bg-primary hover:text-white transition-colors <?php echo $index === 0 ? 'rounded-t-[20px]' : ''; ?> <?php echo $index === count($tabs_resources) - 1 ? 'rounded-b-[20px]' : ''; ?>" data-tab="tab-<?php echo $index; ?>">
    ...
    <!-- Desktop Tabs -->
    <div class="hidden lg:flex justify-start gap-[60px] mb-12 flex-wrap border-b-[2px] border-tertiary" data-aos="fade-up">
        <?php foreach ($tabs_resources as $index => $tab): 
            $tab_title = $tab['tab_title'];
        ?>
            <button type="button" class="tab-button transition-all <?php echo $index === 0 ? 'active ' : 'text-foreground '; ?>" data-tab="tab-<?php echo $index; ?>">
                <?php echo esc_html($tab_title); ?>
            </button>
        <?php endforeach; ?>
    </div>
    ...
    ?>
        <div class="tab-content border-[2px] border-tertiary p-[20px] md:p-[35px] lg:p-[56px] <?php echo $index !== 0 ? 'hidden' : ''; ?>" id="tab-<?php echo $index; ?>">
            <?php if ($tab_resources) : ?>
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-x-8 md:gap-x-[40px] gap-y-8 lg:gap-y-[40px]" data-aos="fade-up">
                    <?php foreach ($tab_resources as $resource): 
                        $title = $resource['title'];
                        $text = $resource['text'];
                        $buttons_cta = $resource['buttons_cta'];
                    ?>
                        <div class="event-card flex flex-col h-full justify-between align-items-center">  
                            <div class="event-conten bg-accent group pt-[35px] lg:pt-[37px] px-[35px] pb-[35px] lg:pb-[40px] flex flex-col flex-grow rounded-[20px]">  
                                <div class="resource-title flex-grow">
                                    <div class="text-foreground mb-0 text-center">
                                        <p class="mb-0 pb-0 font-bold"><?php echo esc_html($title); ?></p>
                                    </div>
                                    <?php if($text): ?>
                                        <div class="text-foreground text-small mt-[5px] text-center"><?php echo $text; ?></div>
                                    <?php endif; ?>
                                </div>
                                <!-- CTA Resources -->
                                <?php if ($buttons_cta) : ?>
                                <div class="flex flex-wrap gap-2 lg:gap-[16px] mt-[20px] justify-center">
