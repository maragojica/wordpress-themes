...
$content_block = get_field('content_block_past_events');
if ($content_block) {    
    ...
    $events_display = $content_block['events_display']; // Button group: 'last_3' or 'all'
    ...
    // Set posts per page based on events_display selection
    $posts_per_page = ($events_display === 'all') ? -1 : 3;
    ...
    $count_allpost = $past_events->post_count;
?>

<section class="past-events-section max-w-full relative overflow-hidden  <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?>>
    

    <div class="container mx-auto relative z-[2]">
        
        <!-- Header Section -->
         <div class="text-left mb-12 w-full lg:w-[70%]" data-aos="fade-in">
             <?php if($subheading): ?>
        ...
        <?php if ($past_events->have_posts()) : ?>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:gap-x-10 lg:gap-x-12 lg:gap-y-[80px]" data-aos="fade-up">
                ...
                <div class="event-card <?php if( $count_allpost > 6): ?> event-item <?php endif; ?>">
                    ...
                    <div class="event-content group">
                        ...
                        <h4 class="event-title mt-[10px] text-foreground">
                            <a class="group-hover:text-primary"  href="<?php echo esc_url(get_permalink()); ?>" aria-label="<?php echo esc_html($event_name); ?>" title="<?php echo esc_html($event_name); ?>" tabindex="0">
        ...
        <?php else : ?>
            <div class="text-center py-12">
                <p class="text-foreground">No past events to display.</p>
            </div>
        <?php endif; ?>
         <!-- Load More Button -->
          <?php if( $events_display == 'all' && $count_allpost > 6): ?>
        <div class="flex items-center gap-4 lg:gap-8 gap-2 mt-[40px] justify-end" data-aos="fade-up">
            ...
            <div class="relative group sm:w-fit w-full">
                    <a href="" tabindex="0" target="_self" aria-label="Load More" title="Load More" class="load-more-events btn sm:w-fit w-full btn_fill_light">
                        Load More                           
                    </a> 
            </div>     
            </div>        
      ...
      <?php endif; ?>
         <!-- CTA Button -->
        <?php if ($buttons) : ?>
            <div class="flex items-center gap-4 lg:gap-8 gap-2 mt-[40px] justify-end">
                   <!-- Horizontal Line -->
              <div class="flex-1 h-[3px] bg-primary h-line-events sm:inline hidden"></div>
                <div class="button-container-wrap flex lg:flex-row flex-col gap-[20px] sm:w-fit w-full">
