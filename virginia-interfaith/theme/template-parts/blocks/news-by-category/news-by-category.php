...
$content_block = get_field('content_block_news_by_category');
if ($content_block) {    
    ...
    // Build tax_query if a category was selected in ACF (select_category)
    $tax_query = [];
    if (!empty($select_category)) {
        // Normalize to array
        $terms = is_array($select_category) ? $select_category : [$select_category];
        ...
        foreach ($terms as $t) {
            if (is_object($t) && isset($t->term_id)) {
                $term_ids_or_slugs[] = (int) $t->term_id;
            } elseif (is_array($t) && isset($t['term_id'])) {
                $term_ids_or_slugs[] = (int) $t['term_id'];
            } elseif (is_numeric($t)) {
                $term_ids_or_slugs[] = (int) $t;
            } else {
                // fallback to slug if value is not numeric
                $use_slug = true;
                $term_ids_or_slugs[] = (string) $t;
            }
        }

        $tax_query[] = [
            'taxonomy' => 'news-type',
            'field'    => $use_slug ? 'slug' : 'term_id',
            'terms'    => $term_ids_or_slugs,
        ];
    }

    // Query for news posts (either limited or all) and apply taxonomy filter if present
    $args = [
        'post_type'      => 'news',
...
<section class="news-section max-w-full relative overflow-hidden  <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?>>
    ...
    <div class="container mx-auto relative z-[2]">
        ...
        <div class="text-left mb-12" data-aos="fade-in">
             <?php if($subheading): ?>
                    ...
                    <?php endif; ?>                     
                    <?php if($description): ?>
                    <div class="text-foreground style-disc style-disc mt-[15px]" data-aos="fade-in" >                 
      ...
      <?php endif; ?>
        ...
        <?php if ($buttons) : ?>
            <div class="flex items-center gap-4 lg:gap-8 gap-2 mt-[40px] justify-end">
                   <!-- Horizontal Line -->
              <div class="flex-1 h-[3px] bg-primary sm:inline hidden"></div>
                <div class="button-container-wrap flex lg:flex-row flex-col gap-[20px] sm:w-fit w-full">
