...
<header id="masthead" class="site-header fixed shadow-sm bg-white left-0 z-[100] w-full text-white max-w-full justify-center py-4 lg:py-[15px] xl:py-[19px] flex items-center  animate__animated" data-animation="fadeIn" data-duration="0.5s">		 
    ...
    <div class="flex lg:hidden justify-between items-center">
        ...
        <div class="flex items-center">
            <button id="mobile-menu-toggle" class="text-primary focus:outline-none" aria-label="Open mobile menu" aria-controls="mobile-menu" aria-expanded="false">
    ...
    <div id="mobile-menu" class="fixed inset-0 bg-white text-secondary h-screen transform translate-x-full transition-transform duration-300 lg:hidden">
		...
		<?php $logo = get_field('branding', 'option')['desktop_colored_logo'];
			  ...
			  <div class="flex items-center">    
            <button id="mobile-menu-close" class="text-primary focus:outline-none" aria-label="Close mobile menu">
                <svg class="h-8 w-8" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
			...
			<?php if (get_field('header_mobile_menu', 'option')): ?>		
					...
					wp_nav_menu(
						array(
							...
							'container'      => false,
							'menu_class'     => 'flex flex-col text-[20px] font-semibold font-primary uppercase text-primary transition-all ease-in-out duration-300 space-y-4 w-full',
						)
					);
					?>	
				<?php endif; ?>	
