...
if ($content_block) {
    ...
    $icons_list = $content_block['icons_list'];
...
<section class="columns-icons-section max-w-full relative <?php if($add_top_shape): ?> top-shape-r <?php endif;?> <?php if($add_bottom_shape): ?> bottom-shape <?php endif;?> <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?> <?php if($bg_color):?> style="background-color: <?php echo $bg_color;?>" <?php endif; ?>>
    <div class="container mx-auto">           
        <div class="w-full lg:mb-[60px] <?php echo $text_align; ?> animate__animated" data-animation="fadeIn" data-duration="1.5s">      
          <?php if($heading): ?>
        ...
        foreach ($icons_list as $column_icons) :          
             $columns_content = $column_icons['columns_content'];   
              //Columns Text Align Settings
                ...
                <div class="flex flex-col sm:flex-row flex-wrap justify-center items-start mx-auto <?php if($i>1): ?> lg:mt-[60px] <?php endif;?>">   
                    <?php foreach ($columns_content as $column) : 
                        
                        $icon = $column['icon'];
                        $title = $column['title'];
                        $headline_icon = $column['headline'];                 
                        $duration = $animation_delay . 's'; 
                        ?>
                        <div class="<?php echo $select_columns; ?> mt-[1.5em] lg:mt-0 animate__animated" data-animation="fadeBottom" data-duration="<?php echo esc_attr($duration); ?>">
                            ...
                            <?php  if(!empty($icon)): 
                                        echo wp_get_attachment_image(
                                            ...
                                            false,
                                            array(
                                                'class' => 'transition-all duration-300 w-[70px] h-[70px] md:w-[87px] md:h-[87px] object-contain',
                                                'alt' => esc_attr($icon['alt']),
                                            )
                                        );
                                endif;  ?>
                                                    
                                <div class="flex-wrap mt-[19px] flex justify-center items-center">
                                    <?php if($headline_icon): ?>
                                        <h3 class="mb-[15px] text-secondary"><?php echo $headline_icon; ?></h3>
                                    <?php endif; ?>
                                    <?php if($title): ?>
