...
if ($hero) {
    ...
    $bg_overlay = $hero['bg_overlay'];
    $banner_type = $hero['banner_type'];
    $image = $hero['bg_image'];   
    $image_mobile = $hero['bg_image_mobile']; 
    $add_bottom_divider = $hero['add_bottom_divider'];
...
<section class="hero-section max-w-full h-screen relative bg-cover bg-center bg-no-repeat <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?> <?php if(!empty($image)):?>style="background-image: url(<?php echo esc_url($image['url']); ?>)"<?php endif; ?>>
    <?php if($banner_type['value'] == "video"){ 
            ...
            <?php } ?>              
    <div class="overlay <?php echo esc_attr($container_classes); ?> absolute z-[2] top-0 left-0 pb-[70px]" <?php if( $bg_overlay): ?> style="background-color: <?php echo $bg_overlay;?>"<?php endif;?> >
     <div class="container mx-auto"> 
        <?php if ($heading) : ?>
           <div class="relative">
            <?php if( $subheading): ?>
                <div class="text-white subheading font-secondary capitalize rotate--10.166 absolute -left-[1.2rem] md:-left-[34px] lg:-left-[44px] -top-[52px] lg:-top-[59px] 2xl:-top-[62px] z-[1] animate__animated" data-animation="fadeIn" data-duration="1.3s"><?php echo $subheading; ?></div>
             <?php endif; ?>
           ...
           <?php endif; ?>  
           <?php if($description): ?>
                <div class="text-sub text-white mt-[1.5em] style-disc lg:max-w-[55%] xl:max-w-[50%] <?php if($alignment_hori == 'middle'): ?> mx-auto <?php endif; ?> animate__animated" data-animation="fadeIn" data-duration="1.3s" >                 
                    <?php echo $description; ?>                   
                </div>
           <?php endif; ?> 
       
     <?php if ($buttons) : ?>
            <div class="flex flex-wrap <?php if($alignment_hori == 'middle'){ ?>justify-center <?php }else{ ?> justify-start <?php } ?> gap-4 mt-6 animate__animated" data-animation="fadeIn" data-duration="1.5s">
