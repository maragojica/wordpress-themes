<?php 
if (have_rows('full_projects')) :          
    while (have_rows('full_projects')) : the_row();
...
<section <?php if($section_id): ?> id="<?php echo $section_id;?>" <?php endif;?> class="section-recent-projects p-t0 p-b0 w-margin <?php if(!$margin_top_desktop): echo ' m-t0'; endif; if(!$margin_bottom_desktop): echo ' m-b0'; endif; if(!$margin_top_mobile): echo ' m-mv-t0'; endif; if(!$margin_bottom_mobile): echo ' m-mv-b0'; endif; ?>" >            
    ...
    <div class="section-video wow fadeIn flex" <?php if(!empty($image)): ?>style="background-image: linear-gradient(to bottom, rgba(0, 0, 0, 0.3), rgba(0, 0, 0, 0.3)), url('<?php echo esc_url($image); ?>');"<?php endif; ?> data-wow-duration="0.8s" data-wow-delay="0.2s">
            <div class="container">
                <div class="title-video-lightbox wow fadeInUp" data-wow-duration="0.6s" data-wow-delay="0.3s">
            <?php if ($title) : ?>
                    <h4 class="cl-white text-uppercase mt-10px mb-10px"><?php echo $title ?></h4>
                <?php endif; ?>
                <?php if($subtitle): ?>
