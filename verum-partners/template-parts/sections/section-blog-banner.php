<?php
if (have_rows('blog_banner')) {
    while (have_rows('blog_banner')) {
        the_row(); 
        ...
        <section class="page-internal-hero p-t0 p-b0 flex" <?php if($bg_overlay && !empty($banner_image)){ ?>style="background: linear-gradient(<?php echo $bg_overlay;?>, <?php echo $bg_overlay;?>), url('<?php echo esc_url($banner_image['url']); ?>') center center no-repeat;" <?php } else{ ?>style="background-color: <?php echo $bg_overlay;?>"<?php } ?>>            
                ...
                <div class="container h-100 flex flex-column center-xs center-xs text-center">
                    <div class="row center-xs">
                       ...
                       <?php if ( !empty($logo)) : 
                        ...
                        <?php endif; ?>  
                           <?php if($headline): ?>
                                <h1 class="cl-white text-uppercase mt-0 mb-02 wow fadeInUp" data-wow-duration="0.8s" data-wow-delay="0.3s"><?php echo $headline; ?></h1>
                           <?php endif; ?>   
                           <?php if($description): ?>
                               <div class="dp-2 text-caps cl-white wow fadeInUp" data-wow-duration="0.8s" data-wow-delay="0.3s"><?php echo $description; ?></div>
                            <?php endif; ?> 
                           <?php if( $cta ):
                            $link_url = $cta['url'];
                            ...
                            $link_target = $cta['target'] ? $cta['target'] : '_self'; ?>     
                             ...
                             <a tabindex="0" class="cta-button cl-white cl-orange-h wow fadeInUp" data-wow-duration="0.8s" data-wow-delay="0.4s" href="<?php echo esc_url( $link_url ); ?>" target="<?php echo esc_attr( $link_target ); ?>" aria-label="<?php echo esc_html( $link_title ); ?>" title="<?php echo esc_html( $link_title ); ?>">
