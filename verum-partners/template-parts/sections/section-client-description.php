<?php 
if (have_rows('client_prof_desc')) :          
    while (have_rows('client_prof_desc')) : the_row();
    ...
    <div class="container">     
        <div class="row middle-xs">
            ...
            <div class="col-xs-12 col-lg-7">                 
                    <div class="info-list-row row">
                        <?php 
                        if (have_rows('info_content')):  $animation_delay = 0.1;
                        while (have_rows('info_content')): the_row();                
                            $title = get_sub_field('title'); 
                            $text = get_sub_field('text');
                            $image = get_sub_field('profile_logo');  
                            $duration = $animation_delay . 's'; ?>
                            <div class="col-xs-12 col-lg-12 m-t2 wow fadeInUp" data-wow-duration="0.8s" data-wow-delay="<?php echo esc_attr($duration); ?>" >
                                <div class="info-list-col profile-list profile-question w-100 h-100 pe-xl-3 ps-xl-3">                       
                                    <?php if ( $title ) :   ?> 
                                        <div class="title-question cl-orange text-uppercase mb-0"><?php echo $title;?></div>
                                    <?php endif; ?> 
                                    <?php if ($text) : ?>
                                ...
                                <img class="verum-profile-logo" src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" height="44" srcset="<?php echo esc_attr($srcset); ?>" sizes="<?php echo esc_attr($sizes); ?>"/>                            
                            <?php endif; ?>
                                </div>                       
                            </div>
