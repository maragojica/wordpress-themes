...
if (have_rows('main_banner')) {
    while (have_rows('main_banner')) {
        ...
        <section class="page-default-hero p-t0 p-b0 flex" <?php if($bg_overlay && !empty($banner_image)){ ?>style="background: linear-gradient(<?php echo $bg_overlay;?>, <?php echo $bg_overlay;?>), url('<?php echo esc_url($banner_image['url']); ?>') center center no-repeat;" <?php } else{ ?>style="background-color: <?php echo $bg_overlay;?>"<?php } ?>>            
