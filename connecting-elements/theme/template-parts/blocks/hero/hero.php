...
<section class="hero-section max-w-full h-[27em] lg:h-[40em] xl:h-[calc(100vh-64px)] relative rounded-[0px_0px_50px_50px] bg-cover bg-center bg-no-repeat <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?> <?php if(!empty($image)):?>style="background-image: url(<?php echo esc_url($image['url']); ?>)"<?php endif; ?>>
    <?php if($banner_type['value'] == "video"){ 
            ...
            <?php } ?>
            <?php if($photo_credit):?>
                <div class="absolute z-[3] right-[30px] xl:right-[80px] md:bottom-4 text-white font-primary-font text-[18px] font-[400] hidden md:block"><?php echo $photo_credit;?></div>
             <?php endif; ?>   
    <div class="overlay <?php echo esc_attr($container_classes); ?> absolute z-[2] top-0 left-0 bg-[linear-gradient(0deg,rgba(0,0,0,0)_81.69%,rgba(0,0,0,0.6)_100%)]" >
     ...
     <?php if ($buttons) : ?>
        ...
        <?php endif; ?>   
        <?php if ($heading) : ?>
            <h1 class="text-secondary max-w-fit xl:max-w-[65%] 3xl:max-w-fit bg-white rounded-tr-[50px] pt-[35px] lg:pt-[40px] pl-[20px] pr-[1.5em] lg:pr-[80px] -mb-[1px] animate__animated" data-animation="fadeIn" data-duration="1.2s">
                <?php echo $heading; ?>
            </h1>
        <?php endif; ?>       
         </div>
