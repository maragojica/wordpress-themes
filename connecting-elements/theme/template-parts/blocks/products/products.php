<?php
$content_block = get_field('products_info');
if ($content_block) {
    ...
    $selected_categories = $content_block['category_product'] ? (array) $content_block['category_product'] : [];
    ...
    if ($categories_as_filters && $search_filter) {
        ...
        $row_class = 'h-full flex flex-col lg:flex-row items-center gap-y-[32px] lg:gap-y-0 lg:gap-x-[32px] row-filter justify-end mt-[2em] lg:mt-[70px]';
   ...
   //Columns Number
    ...
    wp_enqueue_script('products', get_template_directory_uri() . '/js/products.js', array('jquery'), CONNECTING_ELEMENTS_VERSION, true);

    // Localize script with data for AJAX requests
   wp_localize_script('products', 'productInfo', array(
       'rest_url' => rest_url(),
...
<section id="products" class="products-section max-w-full relative <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?> <?php if($bg_color):?> style="background-color: <?php echo $bg_color;?>" <?php endif; ?>>
    <div class="w-full  pl-[22px] lg:pl-0 pr-[22px] lg:pr-0 mx-auto">           
        ...
        <div class="row-product-filter relative z-[4] <?php echo $row_class;?> animate__animated" data-animation="fadeIn" data-duration="1.5s">
                ...
                <?php if ($categories_as_filters && is_array($selected_categories) && !empty($selected_categories)) : ?>
                        <div class="categories-filters select-dropdown pr-[68px] text-right <?php if($hide_filters): ?> hide <?php endif; ?> <?php echo $filter_col_class;?>">
                            <button href="#" role="button" data-value="" class="select-dropdown__button dropdown__button__blog"><span>Product Categories </span> <i class="chevron-down"></i>
                                </button>
       ...
       <?php 
        if ($query->have_posts()) : ?>        
            <div id="main-product" class="product product-items flex flex-col sm:flex-row flex-wrap justify-center mt-6 animate__animated" data-animation="fadeBottom" data-duration="1s">   
                <?php  while ($query->have_posts()) : $query->the_post();
                            $title = get_the_title(); 
                            $id = get_the_id(); 
                            $brand = get_field('brand', $id);    ?>  
                        <div class="<?php echo $select_columns; ?> product-item content-product">
                            <div class="w-full h-full group cursor-pointer modal-trigger" data-modal-id="modal-product-<?php echo $id; ?>">
                            <div class="w-full h-[300px] 2xl:h-[412px] 4k:h-[550px] relative overflow-hidden rounded-[0px_50px_0px_50px]" >                            
                                ...
                                <?php if($title): ?>
                                    <div class="bg-white text-right rounded-tl-[50px] absolute bottom-0 right-0 z-[2] w-fit -mb-[1px] pl-[30px] 2xl:pl-[43px] pr-[16px] pt-[20px] 2xl:pt-[30px]">
                                        ...
                                        <?php endif; ?>  
                                        <?php if($brand): ?>  
                                            <div class="text-primary text-[20px] xl:text-[24px] font-bold font-primary-font leading-[1.2] mt-1"><?php echo $brand;?></div>
                                        <?php endif; ?>    
                                    </div>
                                 <?php endif; ?>   
                            </div>
        ...
        <?php if ($load_more && $query->found_posts > $number_of_posts) : ?>
            <div class="relative text-center mb-[3em] xl:mb-[88px] mt-[2em] xl:mt-0">               
                <a id="load-more" href="#" tabindex="0" target="_self" aria-label="Load More" title="Load More" class="seeMoreProduct min-w-min btn btn_primary max-w-fit mx-auto">
                    Load More  
                </a>
            </div>
        <?php endif; ?>
...
$queryall = new WP_Query($argsall);
        ...
        <?php  while ($queryall->have_posts()) : $queryall->the_post();
            ...
            $add_photo_credits = get_field('add_photo_credits', $id);
            $photo_credit = get_field('photo_credit', $id);
            $product_slider = get_field('product_slider', $id);   
            $buttons = get_field('buttons', $id);  
            $terms_list = array(); 
             ?>  
             <!-- Modal -->
            <div id="modal-product-<?php echo $id; ?>" class="hidden fixed modal-product inset-0 max-w-full bg-secondary bg-opacity-90 flex items-start justify-center overflow-y-auto lg:p-12 py-12 px-2 z-[99999] transition-opacity duration-300 ease-in-out opacity-0">
                <div class="bg-white w-full 2xl:max-w-[1368px] max-w-fit lg:p-[60px] pt-[72px] pb-[47px] px-[20px] relative overflow-y-auto max-h-[90vh] lg:max-h-max transform scale-95 transition-transform duration-300 custom-scroll">
                    ...
                    <div class="flex flex-col items-start lg:gap-[40px] gap-[5px]">
                        ...
                        <div class="w-full"><?php if($title): ?><h2 class="text-secondary mb-0"><?php echo $title; ?></h2><?php endif; ?></div>     
                            <?php if($brand):?><h3 class="text-primary mt-[10px] mb-3"><?php echo $brand; ?></h3><?php endif; ?>  
                             <?php if($description): ?>
                                <div class="font-text-font text-foreground style-disc mt-[10px] mb-3 lg:pt-4" ><?php echo $description; ?></div>
                              <?php endif; ?>                  
                            <div class="font-text-font text-foreground style-disc mt-[10px] mb-3 lg:pt-4" > 
                                <p class="mb-0"><i>Product Categories:</i>  </p>             
                                 <?php $terms_info = get_the_terms( $id , 'product-category' ); 
                                     if(!empty($terms_info)):
                        ...
                        <div class="w-full lg:w-[70%]">                  
                            <?php if($product_slider){ ?>
                                <div class="slide-products slide-view">             
                                    <div class="uk-position-relative uk-visible-toggle uk-light" tabindex="-1" uk-slider>
                                            <div class="uk-position-relative">
                                                <div class="uk-slider-container uk-light">
                                                    ...
                                                    <?php foreach( $product_slider as $image ): 
                                        ...
                                        <div id="content-slide" class="uk-switcher" >
                                            ...
                                            <div> 
                                                <a class="uk-inline" href="#" >
                                                    ...
                                                    <?php if($add_photo_credits): 
                                                        if($photo_credit): ?>
                                                        <span class="absolute bottom-0 bg-white z-[2] w-fit text-secondary text-[20px] lg:text-[22px] font-[800] font-primary-font leading-[1] -mb-[1px]  right-0 rounded-[50px_0px_0px_0px] p-[20px] pb-0"><?php echo $photo_credit;?></span>
                                                     <?php endif; endif; ?>   
                                                    </div>
                                                </a>
                                            </div>
                            ...
                            <?php }else{ ?>
                                ...
                                <?php echo get_the_post_thumbnail($id, 'full', ['class' => 'w-full h-fit object-cover object-center h-[13em] md:h-[20em] lg:h-[25em]']); ?>
                             <?php } ?>    
