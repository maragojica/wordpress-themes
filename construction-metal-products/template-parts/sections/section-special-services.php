...
if (have_rows('section_list_special')) :          
    while (have_rows('section_list_special')) : the_row();
        ...
        <div class="row justify-center"> 
            <?php if($heading ||  $subheading): ?>
                ...
                <?php if ($subheading) : ?>
                    ...
                    <?php endif; ?>      
                    <?php if ($heading) : ?>
                        <h2 class="cl-black mt-0 mb-0 text-uppercase wow fadeInUp" data-wow-duration="0.3s" data-wow-delay="0.3s"><?php echo $heading; ?></h2>
                    <?php endif; ?>  
                </div>
                <?php endif; ?>  
                <?php if($description): ?>
            ...
            <?php 
            if (have_rows('boxes_list')):  $animation_delay = 0.1; ?>
            <div class="info-list-row row text-center justify-center">
             <?php  while (have_rows('boxes_list')): the_row();                
                $title = get_sub_field('title'); 
                $image = get_sub_field('image');
                $cta = get_sub_field('cta');       
                $duration = $animation_delay . 's'; ?>
                <div class="col-xs-12 col-md-6 col-lg-4 m-t2 wow fadeInUp" data-wow-duration="0.3s" data-wow-delay="<?php echo esc_attr($duration); ?>" >
                      <div class="box-resources middle-xs w-100 h-100"> 
                            <?php if ($title) : ?>
                                <div class="box-header-resources"><h3 class="cl-white text-uppercase m-t0 m-b0 text-center cl-navy"><?php echo wp_kses_post($title); ?></h3></div>
                            <?php endif; ?> 
                            <?php if( $cta ):
                            $link_url = $cta['url'];
                            $link_title = $cta['title'];
