...
$content_block = get_field('content_block_insights');
if ($content_block) {    
    ...
    $news_display = $content_block['insights_display']; // Button group: 'last_3' or 'all'
    ...
    // Set posts per page based on news_display selection
    $posts_per_page = ($news_display === 'all') ? -1 : 3;

    // Query for past events (most recent 3 or all)
    $news = new WP_Query([
        'post_type' => 'post',
        'posts_per_page' => $posts_per_page,
        'orderby' => 'date',
        'order' => 'DESC' // Organize by post date, most recent first
    ]);    
   
    $count_allpost = $news->post_count;
?>

<section class="insights-section max-w-full relative overflow-hidden  <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?>>
   
    <div class="container mx-auto relative z-[2]">
        
        <!-- Header Section -->
        <div class="text-left mb-12" data-aos="fade-in">
             <?php if($subheading): ?>
                    ...
                    <?php endif; ?>                     
                    <?php if($description): ?>
                    <div class="text-foreground style-disc style-disc mt-[20px] lg:w-[63%]" data-aos="fade-in" >                 
        ...
        <?php if ($news->have_posts()) : ?>
            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 md:gap-x-10 lg:gap-x-12 gap-y-8 lg:gap-y-[80px]" data-aos="fade-up">
                ...
                <div class="event-card news-card <?php if( $count_allpost > 6): ?> news-item <?php endif; ?>">
                            <?php if ($featured_image && !empty($featured_image['url'])) : ?>
                                <div class="event-image relative overflow-visible">
                                    <a class="block w-full h-full relative z-[2]" href="<?php echo esc_url(get_permalink()); ?>" aria-label="<?php echo esc_html($news_title); ?>" title="<?php echo esc_html($news_title); ?>" tabindex="0">
                        ...
                        <h4 class="event-title mt-[10px] text-primary">
                            <a class="group-hover:text-secondary"  href="<?php echo esc_url(get_permalink()); ?>" aria-label="<?php echo esc_html(get_the_title()); ?>" title="<?php echo esc_html(get_the_title()); ?>" tabindex="0">
        ...
        <?php else : ?>
            <div class="text-center py-12">
                <p class="text-foreground">No past news to display.</p>
            </div>
        <?php endif; ?>
         <!-- Load More Button -->
          <?php if( $news_display == 'all' && $count_allpost > 6): ?>     

       <div class="flex items-center gap-4 lg:gap-8 gap-2 mt-[40px] justify-center" data-aos="fade-up">           
            ...
            <div class="relative group sm:w-fit w-full">
                    <a href="" tabindex="0" target="_self" aria-label="Load More" title="Load More" class="load-more-news btn sm:w-fit w-full btn_outline_light">
                        Load More                           
                    </a> 
            </div>     
            </div>        
      ...
      <?php endif; ?>
         <!-- CTA Button -->
        <?php if ($buttons) : ?>
            <div class="flex items-center gap-4 lg:gap-8 gap-2 mt-[40px] justify-center">
                   <!-- Horizontal Line -->
