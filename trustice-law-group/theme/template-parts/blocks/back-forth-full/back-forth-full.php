<?php
$content_block = get_field('content_block_full');
if ($content_block) {
    
    $bg_color = $content_block['bg_color'];
     $add_top_separator = $content_block['add_top_separator'];
      $add_bottom_separator = $content_block['add_bottom_separator'];
    $add_media_top_overlap = $content_block['add_media_top_overlap'];  
    $add_media_bottom_overlap = $content_block['add_media_bottom_overlap'];  
    ...
    switch ($columns_size) {
         case 'small':
            // 30/70 on medium and up, full width stacked on small
            $heading_width = 'w-full lg:w-[30%] flex flex-col'; 
            $content_width = 'w-full lg:w-[70%] flex flex-col';  
            break;
         case 'small-reverse':
            // 70/30 on medium and up, full width stacked on small
            $heading_width = 'w-full lg:w-[70%] flex flex-col'; 
            $content_width = 'w-full lg:w-[30%] flex flex-col';  
            break;    
        case 'equal':
    ...
    switch ($columns_gap) {
        ...
        case 'medium':
            if ($reverse_desktop) {
                $columns_gap_class = ' lg:pl-[30px] xl:pl-[54px] lg:pr-0';
            } else {
                $columns_gap_class = ' lg:pr-[30px] xl:pr-[54px] lg:pl-0';
            }
            break;
        case 'large':
            if ($reverse_desktop) {
        ...
        case 'xlarge':
            if ($reverse_desktop) {
                $columns_gap_class = ' lg:pl-[95px] xl:pl-[160px] lg:pr-0';
            } else {
                $columns_gap_class = ' lg:pr-[95px] xl:pr-[160px] lg:pl-0';
            }
            break;
        default:   
            if ($reverse_desktop) {
    ...
    switch ($row_gap) {
        case 'small':
            ...
            break;
        case 'medium':
            $row_gap_class = ' lg:py-[34px] ';
            break;
        case 'large':
            $row_gap_class = ' lg:py-[48px] ';
            break;
        case 'xlarge':
            $row_gap_class = ' lg:py-[100px] ';
            break;
        default:   
            $row_gap_class = ' lg:py-[48px] ';
            break; 
  ...
  $overlap_class = '';
    if($add_media_top_overlap){
        $overlap_class = ' overlap-full-media lg:mt-[40px] xl:mt-[52px] ';
        if($reverse_desktop){
            $overlap_class .= ' overlap-top-reverse lg:left-[70px] xl:left-[104px] ';
        }else{
            $overlap_class .= ' overlap-top-normal lg:right-[70px] xl:right-[104px] ';
        }
    }
    if($add_media_bottom_overlap){
        $overlap_class = ' overlap-full-media lg:mb-[40px] xl:mb-[52px] ';
      if($reverse_desktop){
            $overlap_class .= ' overlap-bottom-reverse lg:left-[70px] xl:left-[104px] ';
        }else{
            $overlap_class .= ' overlap-bottom-normal lg:right-[70px] xl:right-[104px] ';
        }
    }              
  $h_separator =  '/shapes/h-pattern.svg';
?>

<section class="back-forth-full-section max-w-full relative overflow-x-hidden <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>   <?php echo esc_attr($className); ?>" <?php echo $anchor_attr; ?>>   
    
    <?php if($add_top_separator){?>
        <div class="w-[100%] leading-[0] mb-[2em] lg:mb-[67px]">
             <?php 
                $svg = file_get_contents(get_template_directory() . $h_separator);
                echo $svg;
                ?>
        </div>
    <?php } ?> 
        <div class="w-full mx-auto relative z-2 <?php if($bg_color): echo $bg_class; endif; ?> <?php if($reverse_desktop){ echo " pr-contain not-rp";}else{ echo " pl-contain not-lp";} ?>">        
          <div class="<?php echo esc_attr($container_classes); ?> lg:gap-y-0 gap-y-[3em]">               
              <div class="<?php echo esc_attr($heading_width); ?>  <?php echo $row_gap_class; ?> <?php echo $columns_gap_class; ?>  justify-center">
                  <div class="px-[2rem] custom-p-info lg:px-0 <?php if($reverse_mobile){ echo " lg:pb-0 pb-[3em]";}else{ echo " lg:pt-0 pt-[3em]";} ?>">
                    <?php if($subheading): ?>
                        <div class="eyebrow text-foreground mb-[12px] lg:mb-[20px]"  >
                            <?php echo $subheading; ?>
                        </div>   
                    <?php endif; ?> 
                        <?php if ($heading) : ?>
                        <?php if($heading_type === 'headthree'): ?>
                            <h3 class="text-tertiary heading-shape mb-0" >
                                <?php echo $heading; ?>
                            </h3>
                        <?php elseif($heading_type === 'headtwo'): ?>
                            <h2 class="text-tertiary heading-shape mb-0" >
                                <?php echo $heading; ?>
                            </h2> 
                    <?php elseif($heading_type === 'headone'):?>
                         <h1 class="text-tertiary heading-shape mb-0" >
                                <?php echo $heading; ?>
                          </h1> 
                    <?php endif;  endif; ?>                     
                        <?php if($description): ?>
                        <div class="text-foreground text-large style-disc mt-[20px] xl:mt-[34px]" data-aos="fade-in" >                 
                    ...
                    <?php if($add_list_items): 
                        ...
                        <div class="grid <?php echo esc_attr($class_list); ?> gap-y-[20px] lg:gap-x-[60px] mt-[20px] xl:mt-[40px]" data-aos="fade-in">
                            <?php if($list_items): foreach ($list_items as $item) : $title = $item['item']; $icon = $item['icon']; ?>
                                <div class="list-item-term flex items-center gap-x-[13px]">
                                    <?php if (!empty($icon)): ?>
                                        <img src="<?php echo $icon['url']; ?>" alt="<?php echo esc_attr($icon['alt']); ?>" class="w-[20px] h-[20px]"/>
                                    <?php endif; ?>
                      ...
                      <?php endif; ?>           
                    <?php if ($buttons) : ?>
                        <div class="flex flex-wrap gap-2 lg:gap-[16px] mt-[30px] xl:mt-[40px] justify-start" data-aos="fade-up">
                                    ...
                                    <a href="<?php echo esc_url($url); ?>" tabindex="0" target="<?php echo esc_attr($target); ?>" aria-label="<?php echo esc_html($title); ?>" title="<?php echo esc_html($title); ?>" class="btn sm:w-fit w-full <?php if($button_style): echo $button_style; endif;?>">
                                        ...
                                        <?php if ($is_external){ ?>
                                            <span class="external-link-icon pl-[10px]" aria-hidden="true">
                                                ...
                                                <path d="M0 12V1.9432H6.94225V3.33573H1.39085V10.6083H9.24745V6.3752H10.6391V12H0ZM4.85345 6.45547L10.4448 1.39173H7.49417V0H13L12.9446 0.088V5.528H11.554V2.2368L5.78699 7.48747L4.85345 6.45547Z" fill="#3e0f28"/>
                                                </svg>
                                            </span>
               ...
               <div class="<?php echo esc_attr($content_width); ?>" <?php if($reverse_desktop){ ?> data-aos="fade-left" <?php }else{ ?> data-aos="fade-right" <?php } ?>  >  
                    <div class="media-box-full w-full lg:min-h-[23em] lg:h-full md:h-[40em] sm:h-[30em] h-[23em] relative bg-cover bg-top bg-no-repeat <?php if($add_media_bottom_overlap || $add_media_top_overlap) { echo $overlap_class; } ?>" <?php if($media_type['value'] == "image"){ ?> style="background-image: url('<?php echo esc_url($image['url']); ?>');" <?php } ?>>
                        <?php if($media_type['value'] == "video"){   ?>   
                                ...
                                <?php if($video_type['value'] == "file"){  ?>   
                                    ...
                                    <?php }elseif($video_type['value'] == "youtube"){   ?>   
                                        <div class="player" data-plyr-provider="youtube" data-plyr-embed-id="<?php echo $youtube_id;?>" <?php if( !empty($poster) ): ?> data-poster="<?php echo esc_url($poster['url']); ?>" <?php endif; ?>></div>

                                        <?php }elseif($video_type['value'] == "vimeo"){   ?>   
                                            <div class="player" data-plyr-provider="vimeo" data-plyr-embed-id="<?php echo $vimeo_id;?>" <?php if( !empty($poster) ): ?> data-poster="<?php echo esc_url($poster['url']); ?>" <?php endif; ?>></div>
                                    <?php } ?>  
                            <?php } ?>    
                    </div>
                </div>   
      ...
      <?php if($add_bottom_separator){?>
        <div class="w-[100%] leading-[0] mt-[2em] lg:mt-[67px]">
             <?php 
                $svg = file_get_contents(get_template_directory() . $h_separator);
                echo $svg;
                ?>
