<?php
$content_block = get_field('content_block_hero');
if ($content_block) {
    ...
    $background_image = $content_block['bg_image']; 
...
<section class="hero-section max-w-full relative overflow-hidden <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>   <?php echo esc_attr($className); ?>" <?php echo $anchor_attr; ?>>   
    <div class="w-full mx-auto absolute z-1 bg-cover bg-top bg-no-repeat h-[560px]" <?php if(!empty($background_image)): ?> style="background-image: url('<?php echo esc_url($background_image['url']); ?>');" <?php endif; ?> >
        <div class="bg-primary opacity-90 w-full h-full">
        </div>
    </div>
   <div class="w-full mx-auto relative z-2 lg:pt-[86px] pt[3em] text-center" >      
         <div class="container mx-auto">            
                <?php if($main_heading): ?>
                        <h1 class="text-white mb-0"  data-aos="fade-up">
                            <?php echo $main_heading; ?>
                        </h1>   
                    <?php endif; ?> 
                    <div class="bg-secondary h-[4px] w-full max-w-[660px] my-[27px] lg:my-[40px] mx-auto"></div>
                    <?php if(!empty($image)): ?>
                        <div class="w-full lg:max-w-[80%] mx-auto mb-[30px] lg:mb-[40px] 2xl:h-[500px] lg:h-[424px] md:h-[40em] sm:h-[30em] h-[23em]">
                            <img src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" class="w-full h-full object-cover object-top skip-lazy"/>
                        </div>
                    <?php endif; ?>
                        <?php if ($heading) : ?>                       
                            <h2 class="text-tertiary mb-0" >
                                <?php echo $heading; ?>
                            </h2>
                     <?php endif; ?>                                  
                        <?php if($description): ?>
                        <div class="text-foreground text-large style-disc mt-[22px] mx-auto max-w-[80%]" data-aos="fade-in" >                 
                            <?php echo $description; ?>                   
                        </div>
                    <?php endif; ?>                                 
                    <?php if ($buttons) : ?>
                    <div class="flex flex-wrap gap-2 lg:gap-[16px] mt-[30px] xl:mt-[36px] justify-center" data-aos="fade-up">
