<?php
$content_block = get_field('content_block_cards_icons');
if ($content_block) {   
    ...
    $buttons_section = $content_block['buttons_section'];        
    ...
    $container_classes = 'flex flex-col w-full';
    $btn_align = 'justify-center';

    //Info Container Settings - FIXED VERSION
    // Set the number of columns
    $num_cols = '3'; // default to 3
    ...
    // Set gap classes with proper Tailwind utilities
    $gap_class = 'gap-y-[35px] md:gap-y-[63px]'; // Default vertical gap
    switch ($columns_gap) {
        case 'small':
            $gap_class .= ' md:gap-x-[15px] xl:gap-x-[20px]';
            break;
        case 'medium':
            $gap_class .= ' md:gap-x-[25px] xl:gap-x-[40px]';
            break;
        case 'large':
            $gap_class .= ' md:gap-x-[35px] xl:gap-x-[80px]';
            break;       
        default:
            $gap_class .= ' md:gap-x-[25px] xl:gap-x-[40px]';
            break;
...
<section class="cards-icons-section max-w-full h-auto relative overflow-hidden <?php echo esc_attr($className); ?> " <?php echo $anchor_attr; ?>>
    ...
    <?php } ?> 
    <div class="relative overflow-hidden mt-[-3px] mb-[-3px] <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>">
      <div class="container mx-auto"> 
          <div class="<?php echo esc_attr($container_classes); ?>">
              <div class="w-full lg:w-[70%] text-center mx-auto">
                  <?php if($subheading): ?>
                        <div class="eyebrow text-secondary mb-[12px]"  >
                            <?php echo $subheading; ?>
                        </div>   
                    <?php endif; ?> 
                        <?php if ($heading) : ?>
                         <h3 class="text-tertiary mb-0" >
                                <?php echo $heading; ?>
                            </h3> 
                    <?php endif; ?>                     
                    <?php if($description): ?>
                    <div class="text-foreground style-disc style-disc mt-[20px]" data-aos="fade-in" >                 
          ...
          <?php if( $info_list ): ?>            
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-<?php echo $num_cols; ?> <?php echo esc_attr($gap_class); ?> mt-[46px]">
                 <?php $i = 0; foreach ($info_list as $column): 
                   
                    $title = $column['title']; 
                     $text = $column['text'];      
                     $icon = $column['icon'];                
                    $buttons = $column['buttons_cta'];   ?>
                    <div class="relative" data-aos="fade-up" data-aos-delay="<?= $i * 100 ?>">  
                      <div class="h-full flex flex-col justify-between bg-accent-light shadow-cards px-[20px] lg:px-[44px] pt-[25px] lg:pt-[30px] pb-[44px] items-start text-left relative">
                         <div class="flex-grow">
                        <?php if ($icon) :
                          $icon_url = $icon['url'];
                            $icon_alt = $icon['alt']; ?>    
                            <div class="w-[55px] h-[55px] lg:w-[70px] lg:h-[70px] flex items-start justify-start ">                         
                                <img class="object-contain w-[55px] h-[55px] lg:w-[70px] lg:h-[70px]" src="<?php echo esc_url($icon_url); ?>" alt="<?php echo esc_attr($icon_alt); ?>" title="<?php echo esc_attr($icon_alt); ?>">
                            </div>
                        <?php endif; ?>                                 
                        <?php if ($title) : ?>
                            <h4 class="text-primary mt-[20px] mb-0">
                                <?php echo $title; ?>
                            </h4>
                        <?php endif; ?>
                        <?php if ($text) : ?>
                            <div class="text-foreground text-small mt-[16px] style-disc">
                                <?php echo $text; ?>
                            </div>
                        <?php endif; ?>  
                        </div> 
                        <?php if ($buttons) : ?>                                
                        <div class="flex flex-col w-full flex-wrap gap-2 lg:gap-8 mt-[30px] lg:mt-[40px] justify-center">
                            <?php foreach ($buttons as $button) : ?>
                                ...
                                if ($button_link) :
                                    ...
                                    <a href="<?php echo esc_url($url); ?>" tabindex="0" target="<?php echo esc_attr($target); ?>" aria-label="<?php echo esc_html($title); ?>" title="<?php echo esc_html($title); ?>" class="btn w-full <?php if($button_style): echo $button_style; endif;?>">
         ...
         <?php endif; ?>    
           ...
           <?php if ($buttons_section) : ?>
             <div class="max-w-full mx-auto">
               <div class="flex flex-wrap justify-center gap-2 lg:gap-8 mt-[4em]" data-aos="fade-up" data-aos-delay="100">
               <?php foreach ($buttons as $button) : ?>
                    <?php 
                    $button_link = $button['button'];
                    $button_style = $button['button_style'];
                    ...
                    if ($button_link) :
                        ...
                        $title = $button_link['title'];
                        $target = $button_link['target'] ? $button_link['target'] : '_self';  ?>
                        <div class="relative group w-full sm:w-fit">
                        <a href="<?php echo esc_url($url); ?>" tabindex="0" target="<?php echo esc_attr($target); ?>" aria-label="<?php echo esc_html($title); ?>" title="<?php echo esc_html($title); ?>" class="btn w-full sm:w-fit <?php if($button_style): echo $button_style; endif;?>">
                            <?php echo esc_html($title); ?>
                            <?php if ($is_external){ ?>
                                <span class="external-link-icon pl-[10px]" aria-hidden="true">
                                    <svg xmlns="http://www.w3.org/2000/svg" width="13" height="12" viewBox="0 0 13 12" fill="none">
