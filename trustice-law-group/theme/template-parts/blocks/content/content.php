...
if ($content_block) {
    ...
    $bg_color = $content_block['bg_color'];
     $add_left_separator = $content_block['add_left_separator'];
     $add_right_separator = $content_block['add_right_separator'];
     $full_width_content = $content_block['full_width_content'];
      $heading_type = $content_block['heading_type'];
    $heading = $content_block['heading'];
    $subheading = $content_block['subheading'];
    $description = $content_block['description'];
    $info_text = $content_block['info_text'];
    $add_list_items = $content_block['add_list_items'];
     $list_items_one_column = $content_block['list_items_one_column'];
      $list_items_two_column = $content_block['list_items_two_column'];
    $buttons = $content_block['buttons'];    
      $add_breadcrumbs = $content_block['add_breadcrumbs'];   
    $breadcrumbs = $content_block['breadcrumbs']; 
...
<section class="content-section max-w-full relative overflow-hidden <?php if($bg_color): echo $bg_class; endif; ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?> <?php echo esc_attr($className); ?> " <?php echo $anchor_attr; ?>> 
    <?php if($add_left_separator){?>
        <div class="h-full h-fit leading-[0] absolute top-0 left-0 lg:block hidden">
             <?php 
                $svg = file_get_contents(get_template_directory() . $v_separator_l);
                echo $svg;
                ?>
        </div>
    <?php } ?> 
   <div class="container mx-auto">     
       <div class="w-full mx-auto <?php if(!$full_width_content) echo 'lg:max-w-[80%]'; ?>"> 
       <div class="<?php echo esc_attr($container_classes); ?>">
          <div class="<?php echo esc_attr($heading_width); ?> ">  
             <?php if($add_breadcrumbs && $breadcrumbs): ?>
                    <div class="breadcrumbs mb-[28px]">
                        <?php 
                        ...
                        ?>
                            <span class="breadcrumb-item">
                                <?php if($link): ?>
                                    <a tabindex="0" aria-label="<?php echo $title; ?>" title="<?php echo $title; ?>" href="<?php echo esc_url($url); ?>" target="<?php echo esc_attr($target); ?>"><?php echo esc_html($title); ?></a>
                                <?php else: ?>
             ...
             <?php if($subheading): ?>
                ...
                <?php if ($heading) : ?>
                    ...
                    <?php endif;  endif; ?>                     
                        <?php if($description): ?>
                        <div class="text-foreground text-large style-disc mt-[20px] lg:mt-[34px] <?php if($full_width_content) echo 'lg:max-w-[80%]'; ?>" data-aos="fade-in" >                 
                            <?php echo $description; ?>                   
                        </div>
                    <?php endif; ?>  
                    <?php if($add_list_items): 
                        $class_list = ''; 
                        if($list_items_two_column) {
                            $class_list = 'grid-cols-1 lg:grid-cols-2';
                        } elseif($list_items_one_column) {
                        ...
                        <div class="grid <?php echo esc_attr($class_list); ?> gap-y-[20px] lg:gap-x-[60px] mt-[20px] xl:mt-[40px]" data-aos="fade-in">
                            <?php if($list_items_one_column): foreach ($list_items_one_column as $item) : $title = $item['item']; $icon = $item['icon']; ?>
                            ...
                            <?php endforeach; endif; ?>
                             <?php if($list_items_two_column): foreach ($list_items_two_column as $item) : $title = $item['item']; $icon = $item['icon']; ?>
                                <div class="list-item-term flex items-center gap-x-[13px]">
                                    <?php if (!empty($icon)): ?>
                                        <img src="<?php echo $icon['url']; ?>" alt="<?php echo esc_attr($icon['alt']); ?>" class="w-[20px] h-[20px]"/>
                                    <?php endif; ?>
    ...
    <?php if($add_right_separator){?>
        <div class="h-full h-fit leading-[0] absolute top-0 right-0 lg:block hidden">
             <?php 
                $svg = file_get_contents(get_template_directory() . $v_separator_r);
                echo $svg;
                ?>
        </div>
    <?php } ?> 
