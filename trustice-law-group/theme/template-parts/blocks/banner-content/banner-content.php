<?php
$content_block = get_field('content_block_banner_content');
if ($content_block) {
       
    $add_content_top_overlap = $content_block['add_content_top_overlap'];  
    $add_content_bottom_overlap = $content_block['add_content_bottom_overlap'];  
     $heading = $content_block['heading'];
    $subheading = $content_block['subheading'];  
    $description = $content_block['description']; 
    $buttons = $content_block['buttons']; 
    $background_image = $content_block['desktop_background_image']; 
    ...
    $container_classes = ' relative flex flex-col lg:flex-row ' . ($reverse_desktop ? 'lg:flex-row-reverse ' : '') . ($reverse_mobile ? 'flex-col-reverse ' : '');
  ...
  $overlap_class = '';
  $overlap_class_section = '';
    if($add_content_top_overlap){
        $overlap_class = ' overlap-banner-content lg:top-[50px] xl:top-[74px] ';    
        $overlap_class_section = ' lg:mb-[50px] xl:mb-[74px] ';   
    }
    if($add_content_bottom_overlap){
        $overlap_class = ' overlap-banner-content lg:bottom-[50px] xl:bottom-[74px] ';   
         $overlap_class_section = ' lg:mt-[50px] xl:mt-[74px] ';      
    }              
  
?>

<section class="banner-content-section max-w-full relative overflow-hidden <?php if($add_content_bottom_overlap || $add_content_top_overlap) { echo $overlap_class_section; } ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>   <?php echo esc_attr($className); ?>" <?php echo $anchor_attr; ?>>   
   <div class="box-banner-media w-full mx-auto relative z-2 lg:bg-cover bg-center bg-no-repeat" <?php if(!empty($background_image)): ?> style="background-image: url('<?php echo esc_url($background_image['url']); ?>');" <?php endif; ?> >
          <div class="<?php echo esc_attr($container_classes); ?> <?php if($add_content_bottom_overlap || $add_content_top_overlap) { echo $overlap_class; } ?>">               
              
             <div class="overlap-content bg-foreground w-full flex flex-col justify-center lg:max-w-[662px] p-[2em] lg:py-[50px] <?php if($reverse_desktop){ echo " lg:pl-[70px] lg:pr-[100px] 2xl:pr-[160px]"; }else{ echo " lg:pr-[70px] lg:pl-[100px] 2xl:pl-[160px]"; } ?>">
                   <?php if($subheading): ?>
                        <div class="eyebrow text-white mb-[12px] lg:mb-[20px]"  >
                            <?php echo $subheading; ?>
                        </div>   
                    <?php endif; ?> 
                        <?php if ($heading) : ?>                       
                     ...
                     <?php endif; ?>    
                     <div class="bg-secondary h-[4px] w-full max-w-[360px] my-[27px]"></div>                 
                        <?php if($description): ?>
                        <div class="text-white style-disc" data-aos="fade-in" >                 
                ...
                <?php if(!empty($mobile_image)): ?> 
                 <div class="media-box w-full lg:hidden block md:h-[40em] sm:h-[30em] h-[23em] relative bg-cover bg-top bg-no-repeat" style="background-image: url('<?php echo esc_url($mobile_image['url']); ?>');" >
                 </div>
                <?php endif; ?>
      </div> 
    </div>      
