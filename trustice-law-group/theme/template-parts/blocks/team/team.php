<?php
$content_block = get_field('content_block_team');
if ($content_block) {
    
    $heading = $content_block['heading'];      
    $subheading = $content_block['subheading']; 
    $description = $content_block['description']; 
    $team_list = $content_block['team_list'];   
    ...
    $container_classes = 'flex flex-col h-full w-full text-center';
    ...
    //Info Container Settings
     ...
     $info_ontainer_classes = 'w-full grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-y-[30px] md:gap-x-[60px] md:gap-y-[60px] ';
?>

<section class="teams-section max-w-full h-auto relative <?php echo esc_attr($bg_class); ?> <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?>>
      <div class="container mx-auto"> 
          ...
          <?php if( $team_list ): ?> 
            <div class="max-w-full mx-auto">
                <div class="<?php echo esc_attr($info_ontainer_classes); ?> <?php echo ($subheading || $heading || $description) ? ' mt-[3em] lg:mt-[44px]' : ''; ?>">
                 <?php foreach ($team_list as $post): 
                    ...
                    <div class="w-full" data-aos="fade-up" >  
                        ...
                        <a class="w-full h-full block" href="<?php the_permalink($post->ID); ?>" tabindex="0" aria-label="<?php echo $title; ?>" title="<?php echo $title; ?>">
                         ...
                         <div class="team-info flex flex-col justify-start w-full h-full text-center group hover:-translate-y-1 transition-transform duration-300">                      
                           <?php 
                                if ( has_post_thumbnail( $post->ID ) ): ?>
                                <div class="w-full h-fit md:h-[25em] lg:h-[440px] 2xl:h-[500px] team-box relative mb-[20px] overflow-hidden">
                                ...
                                ?>
                            <?php if($title): ?>
                                <h5 class="text-primary mb-0 transition-all duration-400 ease-in-out"><?php echo $title;?></h5>
                            <?php endif; ?> 
                             <?php if($position): ?>
                                <div class="text-foreground <?php if($title): ?> mt-0 <?php endif;?>" data-aos="fade-up" >                 
                                    <?php echo $position; ?>                   
                                </div>
                            <?php endif; ?>                            
                       </div>
