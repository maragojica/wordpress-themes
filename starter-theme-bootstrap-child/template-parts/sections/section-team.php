...
$slug = "general";
$list_team_desktop = get_field('list_team_desktop'); 
$list_team_desktop_right = get_field('list_team_desktop_right'); ?>


<?php $query = array(
        'post_type' => array( 'team' ),     //All Team
        'post_status' => 'publish',
        'orderby' => 'menu_order',
        'order' => 'desc',
        'posts_per_page' => -1,
    ...
    <section class="section-team">
        <div class="container-fluid pt-5 pb-5">
            ...
            <div class="wrap-row-team hide-md">
                ...
                <div class="row-team row-team-right hide-md">
                    <?php $i = 1; foreach( $list_team_desktop_right as $post  ):
                        setup_postdata($post);  
                        $bg_color_desktop = get_field('bg_color_desktop'); 
                        $position = get_field('position_team');  
                        $hide_photo_main_list = get_field('hide_photo_main_list');
           ...
           <?php if ( $allpost->have_posts() ) : ?>
            <div class="row-team mb-4 show-md">
                <?php $i = 1; while ($allpost->have_posts()) : $allpost->the_post();  
                    $bg_color_desktop = get_field('bg_color_desktop'); 
                    $position = get_field('position_team');  
                    $hide_photo_main_list = get_field('hide_photo_main_list');
                    $linkedin_team = get_field('linkedin_team'); ?>
            ...
            <?php if( $cta ):
                ...
                <div class="row mb-md-5 pt-md-4 row-cta-lets-talk">
                    <div class="col-md-12">                       
                        <a tabindex="0" class="btn-cta center-btn" data-wow-duration="0.8s" data-wow-delay="0.2s" href="<?php echo esc_url( $link_url ); ?>" target="<?php echo esc_attr( $link_target ); ?>" aria-label="<?php echo esc_html( $link_title ); ?>" title="<?php echo esc_html( $link_title ); ?>"><?php echo esc_html( $link_title ); ?>
                        </a>                            
                    </div>
                </div>
            <?php endif; ?>
