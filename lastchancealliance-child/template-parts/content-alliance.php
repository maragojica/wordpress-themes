<?php
/**
 * Displays the content when the alliance template is used.
 *
 * @package WordPress
 * @subpackage Twenty_Twenty
 * @since 1.0.0
 */

?>
<div class="content-colour-pages bg-dark">
    <section class="banner-colour-page bg-dark">
        <div class="container">
            <div class="row justify-content-center bg-white row-center-content">
                <div class="col-md-11">
                    <?php if( get_field('title_headlines_inner') ): ?>
                        <h3 class="headline-section cl-dark-2"><?php echo the_field('title_headlines_inner'); ?></h3>
                    <?php endif; ?>
                    <div class="box-weglot">
                        <ul class="list-languages">
                            <li id="menu-item-weglot-1283-en" class="weglot-lang menu-item-weglot weglot-language weglot-en menu-item menu-item-type-custom menu-item-object-custom menu-item-weglot-1283-en" data-wg-notranslate=""><a title="English" href="https://lastchancealliance.org/staging/alliance/" data-wg-notranslate="">English</a><span>/</span></li>
                            <li id="menu-item-weglot-1283-es" class="weglot-lang menu-item-weglot weglot-language weglot-es menu-item menu-item-type-custom menu-item-object-custom menu-item-weglot-1283-es"><a title="Español" href="https://lastchancealliance.org/staging/es/alliance/" data-wg-notranslate="true">Español</a></li>
                        </ul>
                    </div>

                    <?php if( get_field('subtitle_headlines_inner') ): ?>
                        <h5 class="subheadline-section cl-dark-blue mt-5 mb-5 pb-2 pb-md-5">
                            <?php echo the_field('subtitle_headlines_inner'); ?>
                        </h5>
                    <?php endif; ?>
                    <?php if( get_field('text_headlines_inner') ): ?>
                        <div class="copy-text sm-text light-content cl-dark-2 mb-5 pb-2 pb-md-5">
                            <?php echo the_field('text_headlines_inner'); ?>
                        </div>
                    <?php endif; ?>
                    <?php $cta = get_field('cta_link_headlines_inner');
                    if($cta) {
                        $link_url = $cta['url'];
                        $link_title = $cta['title'];
                        $link_target = $cta['target'] ? $cta['target'] : '_self'; ?>
                        <div>
                            <a class="cta-link cl-dark-green cl-dark-greenh mt-2" href="<?php echo $link_url; ?>" target="<?php echo $link_target;?>"><?php echo $link_title; ?>
                                <svg class="d-inline-block" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" width="24px" height="34px">
                                    <image  x="0px" y="0px" width="24px" height="34px"  xlink:href="data:img/png;base64," />
                                </svg>
                            </a>
                        </div>
                    <?php } ?>
                        <div class="row">
                            <?php if( get_field('list_organizations_1') ): ?>
                                <div class="col-md list-organizations copy-text sm-text light-content cl-dark-2">
                                    <?php echo the_field('list_organizations_1'); ?>
                                </div>
                            <?php endif; ?>
                            <?php if( get_field('list_organizations_2') ): ?>
                                <div class="col-md list-organizations copy-text sm-text light-content cl-dark-2">
                                    <?php echo the_field('list_organizations_2'); ?>
                                </div>
                            <?php endif; ?>
                        </div>
                    <?php if( get_field('text_policy') ): ?>
                        <div class="row">
                        <div class="col-md text_policy copy-text sm-text light-content cl-dark-2">
                            <?php echo the_field('text_policy'); ?>
                        </div>
                        </div>
                    <?php endif; ?>
                </div>
            </div>
            <div class="row justify-content-center">
                <div class="col-md-10 col-lg-7">
                    <div class="contact-form">
                        <?php gravity_form(3, false); ?>
                    </div>
                </div>
            </div>
        </div>
    </section>
</div>