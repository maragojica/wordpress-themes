...
if ($content_block) {
    ...
    $info_services = $content_block['info_categories']; 
...
<section class="services-section max-w-full relative overflow-hidden lg:h-fit <?php echo esc_attr($className); ?> <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>" <?php echo $anchor_attr; ?> >
    ...
    <?php if ($info_services):  ?>       
    <div class="pt-[60px] md:mx-[-1.5rem] <?php echo esc_attr($container_classes); ?>" data-aos="fade-up" >   
    <?php foreach ($info_services as $column_slider): 
            $heading = $column_slider['heading'];           
            $image = $column_slider['image']; 
            $page_link = $column_slider['page_link'];?>    
            
            <!-- Card -->
            <div class="w-full md:w-1/2 lg:w-1/3 h-[300px] xl:h-[395px] md:px-[15px] mb-[30px]" >
              <div class="relative group w-full h-full overflow-hidden">
                ...
                <?php  endif; ?> 
                <div class="absolute left-0 bottom-0 bg-primary h-[100px] xl:h-[120px] group-hover:h-full group-hover:bg-primary group-hover:backdrop-blur-[37px] w-full flex justify-center items-center text-center transition-all duration-300 ease-in-out">
                    <?php if($heading): ?>
                        <?php  if ($page_link) :
                            $url = $page_link['url'];
                            ...
                            <h3 class="text-white uppercase"><?php echo $heading;?></h3>
