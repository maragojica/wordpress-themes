...
if ($content_block) {
    ...
    $special_subheading = $content_block['special_subheading']; 
    ...
    $heading_width = 'w-full md:w-1/2 xl:w-1/2 2xl:w-2/5'; 
    $content_width = 'w-full md:w-1/2 xl:w-1/2 2xl:w-3/5';      
    $btn_align = 'justify-start';
   
?>

<section class="back-foth-media-full-section max-w-full relative <?php echo esc_attr($className); ?> " <?php echo $anchor_attr; ?>>   
    <div class="w-full mx-auto <?php if($reverse_desktop){ echo " pr-contain not-rp";}else{ echo " pl-contain not-lp";} ?>">     
       <div class="<?php echo esc_attr($container_classes); ?> md:gap-y-0 gap-y-[3em] <?php echo esc_attr($spacing_class); ?> <?php echo esc_attr($spacing_responsive_class); ?>">
          <div class="<?php echo esc_attr($heading_width); ?> px-[1.5rem] <?php if($reverse_desktop){ ?> md:pl-[50px] lg:pl-[80px] <?php }else{ ?> md:pr-[50px] lg:pr-[80px] <?php } ?> flex flex-col justify-center">
            <?php if ($heading) : ?>
                <div class="relative">
                    <?php if( $subheading): ?>
                        <div class="text-sub text-secondary mb-[15px] <?php if($special_subheading):?> special-sub<?php endif;?>" data-aos="fade-up" ><?php echo $subheading; ?></div>
                    <?php endif; ?>
            ...
            <div class="<?php echo esc_attr($content_width); ?>" <?php if($reverse_desktop){ ?> data-aos="fade-left" <?php }else{ ?> data-aos="fade-right" <?php } ?>  >  
                    <?php if($media_type['value'] == "image"){ ?>
                            <?php if ( !empty($image)) : 
                                ...
                                <img class="img-fluid w-full h-full object-center object-cover" src="<?php echo esc_url($image['url']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" height="600" srcset="<?php echo esc_attr($srcset); ?>" sizes="<?php echo esc_attr($sizes); ?>"/>                            
                            <?php endif; ?>   
