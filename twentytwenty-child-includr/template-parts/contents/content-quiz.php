<?php
/**
 * Displays the content when the quiz template is used.
 *
 * @package WordPress
	...
	<div class="section section-main-single mt-lg-0 mt-5">
		<div class="container">
			<div class="row row-center-lg equal ml-md-auto align-items-md-center">
				...
				<div class="col-md-7">
					<div class="copy-text-lg cl-dark-3 text-md-left text-center"><?php the_content(); ?></div>
					<div class="row equal justify-content-sm-between align-items-center">
						<div class="col-lg-7 text-md-left text-center">
							<div class="d-flex flex-column flex-md-row justify-content-between align-items-center">
								<div>
									<?php $cat_quiz = get_field('category_quiz');
									if( $cat_quiz ){ ?>
									<h4 class="title-info title-info-cate cl-red pb-3"><?php echo $cat_quiz; ?></h4>
									<?php } ?>
								...
								<div class="quote-author quote-author-sm d-flex align-items-center justify-content-center">
									<?php $authorimg = get_field('author_photo_quiz');
									if( !empty($authorimg) ){ ?>
									...
									<div class="pl-3 text-left">
										<?php $author = get_field('author_name_quiz');
										if( $author ){
											$author_name = $author;
					...
					<div class="quiz-info pod-description mb-md-5 pt-4">
						<div class="copy-text copy-blockquote cl-dark-3 mt-4">
							<div class="quiz">
								<?php
								if( have_rows('list_quiz') ):
									$i = 1;
									while( have_rows('list_quiz') ) : the_row();
									?>
									<h2 class="quiz-question"><?= $i ?>. <?= get_sub_field('question_quiz'); ?></h2>
									<img class="img-fluid w-100 mb-5" src="<?= get_sub_field('image_quiz'); ?>">
									<?php
									if( have_rows('answers_list') ): $j = 1;
									?>
									<ul data-quiz-question="<?= $i; ?>">
										<?php
										while( have_rows('answers_list') ) : the_row();
											$ans = get_sub_field('is_correct')?"yes":"no";
											?>
										<li class="quiz-answer" data-quiz-answer="<?= $j ?>" data-answer="<?= $ans ?>"><?= get_sub_field('answer_quiz'); ?></li>
										<?php
										$j++;
										endwhile;
										?>
									...
									?>
										<div class="cl-dark-2 box-info-quiz box-quiz-correct-<?= $i; ?>" style="display: none;">
											<h4 class="cl-green-dark"><?= get_sub_field('title_correct'); ?></h4>
											<?php if(get_sub_field('text_correct')): ?>
												<?php echo get_sub_field('text_correct'); ?>
											<?php endif; ?>
										</div>
										<div class="cl-dark-2 box-info-quiz box-quiz-incorrect-<?= $i; ?>" style="display:none;">
											<h4 class="cl-red"><?= get_sub_field('title_incorrect'); ?></h4>
											<?php if(get_sub_field('text_incorrect')): ?>
												<?php echo get_sub_field('text_incorrect'); ?>
											<?php endif; ?>
										</div>
									<hr>
									<?php
							...
							<div id="quiz-result-wrap" class="quiz-result-wrap" style="display:none">
								<div><?php echo get_field('title_score'); ?></div>
								<div class="wrap-score">
									<div>
										<div class="quiz-result quiz-result-score" style="background: url(<?php echo get_field('image_score'); ?>) no-repeat;"></div>
									</div>
									<div class="quiz-result-text">
										<?php echo get_field('text_score'); ?>
									</div>
								</div>
							</div>
							...
							var Quiz = function(){
								...
								this._calcResult = function(){
									...
									$('ul[data-quiz-question]').each(function(i){
										...
										id=jQuery(this).data('quiz-question');


										//correctAnswer = $this.data('answer');
										correctAnswer = $this.find('.quiz-answer[data-answer="yes"]').data('quiz-answer');
										...
										if ( chosenAnswer == correctAnswer ) {
											...
											$('.box-quiz-correct-'+id).show();
										}
										else {
											$this.find('.quiz-answer[data-quiz-answer="'+correctAnswer+'"]').addClass('correct');
											$this.find('.quiz-answer.active').addClass('incorrect');

											$('.box-quiz-incorrect-'+id).show();
										}

										$this.find('.quiz-answer').addClass('disabled');
									});


									return {code: 'good', text: numberOfCorrectAnswers+'/<?php echo ($i-1); ?>'};
								}
								this._isComplete = function(){
									...
									if ( answersComplete >= <?php echo ($i-1); ?> ) {
										return true;
									}
									else {
										return false;
								...
								this._bindEvents = function(){
									$('.quiz-answer').on('click', function(){
										...
										if ( self._isComplete() ) {
											...
											$('html, body').animate({
												scrollTop: $('#quiz-result-wrap').offset().top
											});
