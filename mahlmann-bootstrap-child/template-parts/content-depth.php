<?php
/**
 * Template part for displaying page content in page-depth.php
 *
...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
    <?php if( have_rows('banner_inside') ): ?>
        ...
        <?php endwhile; ?>
    <?php endif; ?>
    <?php if( have_rows('section_cities_list') ): ?>
        <?php while ( have_rows('section_cities_list') ) : the_row();
            $title = get_sub_field('title_section_cities_list');
            $text = get_sub_field('text_section_cities_list');?>
            <section class="section-about pb-3 pt-3">
                <div class="container">
                    <div class="row justify-content-center">
                    ...
                    <?php if( have_rows('images_section_cities_list') ): ?>
                    <div class="row equal justify-content-center row-city">
                        <?php while ( have_rows('images_section_cities_list') ) : the_row();
                            $img = get_sub_field('img_section_cities_list');
                            $size = get_sub_field('size_section_cities_list');?>
                            <div class="col-md-6 <?php if( $size['value'] == "big" ){ ?>col-lg-6<?php }else{?>col-lg-3<?php } ?> mb-4">
                                <div class="box-img-city w-100 h-100">
                                    <?php if( !empty($img) ): ?>
                                        <img class="city-img cover-img img-fluid m-auto h-100 w-100" src="<?php echo esc_url($img['url']); ?>" alt="<?php echo esc_attr($img['alt']); ?>" />
                                    <?php endif; ?>
                                </div>
                            </div>
                        <?php endwhile; ?>
